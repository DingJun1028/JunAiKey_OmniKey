...
export class UtilityAgent extends BaseAgent {
    ...
    protected async handleMessage(message: AgentMessage): Promise<AgentResponse> {
        console.log(`[UtilityAgent] Handling message: ${message.type} (Correlation ID: ${message.correlationId || 'N/A'})`);
        ...
        try {
            ...
            switch (message.type) {
                case 'insert_date':
                    ...
                    result = { status: 'success', date: formattedDate, message: `Inserted date: ${formattedDate}` };
                ...
                case 'apply_text_effect':
                    ...
                    switch (message.payload.effect) {
                        case 'bold': modifiedText = `**${modifiedText}**`; break;
                        case 'italic': modifiedText = `*${modifiedText}*`; break;
                        case 'underline': modifiedText = `<u>${modifiedText}</u>`; break; // HTML underline
                        // Add other effects
                        default: console.warn(`[UtilityAgent] Unknown text effect: ${message.payload.effect}`); break;
                    }
                ...
                // TODO: Add cases for other utility tasks (e.g., calculations, unit conversions, simple text manipulation)
