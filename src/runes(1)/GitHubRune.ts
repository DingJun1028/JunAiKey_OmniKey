...
export class GitHubRune {
    ...
    constructor(context: SystemContext, rune: Rune) {
        ...
        this.rune = rune;
        console.log(`[Rune] GitHubRune initialized for ${rune.name}.`);
    ...
    private async getGitHubPat(userId: string): Promise<string | undefined> {
        ...
        if (!pat) {
             console.warn(`[Rune] GitHub PAT not found for user ${userId}. GitHub operations will likely fail.`);
             this.context.loggingService?.logWarning(`GitHub PAT not found for user ${userId}`, { runeId: this.rune.id, userId });
    ...
    async getUser(params: any, userId: string): Promise<any> {
        console.log(`[Rune] ${this.rune.name}.getUser called by user ${userId}.`);
        this.context.loggingService?.logInfo(`Executing rune action: ${this.rune.name}.getUser`, { runeId: this.rune.id, userId });
        ...
        try {
            ...
            const config = pat ? { headers: { Authorization: `Bearer ${pat}` } } : undefined;
            const result = await this.context.apiProxy?.callGitHubAPI('/user', 'GET', undefined, config);
            // --- End Modified ---

            console.log(`[Rune] ${this.rune.name}.getUser result:`, result);
            return { status: 'success', data: result };

        } catch (error: any) {
            console.error(`[Rune] Error executing ${this.rune.name}.getUser:`, error);
            this.context.loggingService?.logError(`Error executing rune action: ${this.rune.name}.getUser`, { runeId: this.rune.id, userId, error: error.message });
    ...
    async listRepos(params: { type?: 'all' | 'owner' | 'member' }, userId: string): Promise<any> {
        console.log(`[Rune] ${this.rune.name}.listRepos called by user ${userId} with params:`, params);
        this.context.loggingService?.logInfo(`Executing rune action: ${this.rune.name}.listRepos`, { runeId: this.rune.id, userId, params });
        ...
        try {
            // --- Modified: Get PAT securely before calling API ---
            const pat = await this.getGitHubPat(userId);
            const config = pat ? { headers: { Authorization: `Bearer ${pat}` }, params: { type: repoType } } : { params: { type: repoType } };
            const result = await this.context.apiProxy?.callGitHubAPI('/user/repos', 'GET', undefined, config);
            // --- End Modified ---

            console.log(`[Rune] ${this.rune.name}.listRepos result:`, result);
            return { status: 'success', data: result };

        } catch (error: any) {
            console.error(`[Rune] Error executing ${this.rune.name}.listRepos:`, error);
            this.context.loggingService?.logError(`Error executing rune action: ${this.rune.name}.listRepos`, { runeId: this.rune.id, userId, error: error.message });
            throw new Error(`GitHub API call failed: ${error.message}`);
        }
    }
    ...
    async createIssue(params: { owner: string, repo: string, title: string, body?: string, labels?: string[] }, userId: string): Promise<any> {
        console.log(`[Rune] ${this.rune.name}.createIssue called by user ${userId} with params:`, params);
        this.context.loggingService?.logInfo(`Executing rune action: ${this.rune.name}.createIssue`, { runeId: this.rune.id, userId, params: { owner: params?.owner, repo: params?.repo, title: params?.title } });
        ...
        try {
            // --- Modified: Get PAT securely before calling API ---
            const pat = await this.getGitHubPat(userId);
            const endpoint = `/repos/${params.owner}/${params.repo}/issues`;
            const method = 'POST';
            const data = {
                title: params.title,
                body: params.body,
            ...
            console.log(`[Rune] ${this.rune.name}.createIssue result:`, result);
            return { status: 'success', data: result, message: `Issue created: ${result?.html_url || result?.title}` };

        } catch (error: any) {
            console.error(`[Rune] Error executing ${this.rune.name}.createIssue:`, error);
            this.context.loggingService?.logError(`Error executing rune action: ${this.rune.name}.createIssue`, { runeId: this.rune.id, userId, error: error.message });
            throw new Error(`GitHub API call failed: ${error.message}`);
        }
    }
    // --- End New ---
