...
- **EN:** Support dynamic loading, unloading, hot-reloading, and versioned upgrades of plugins at runtime. Provide clear plugin registration/unregistration APIs and lifecycle hooks (onLoad, onUnload, onUpdate).
- **中文：** 支援執行期動態載入、卸載、熱重載與版本升級，提供明確的插件註冊/反註冊 API 與生命週期鉤子（onLoad、onUnload、onUpdate）。
...
- **EN:** Use peerDependencies, semantic versioning, and dependency graph checks. Warn or block incompatible plugins at load time. Document required/optional dependencies in plugin manifest.
- **中文：** 使用 peerDependencies、語意化版本與依賴圖檢查，載入時警告或阻擋不相容插件，並於插件描述檔明確標註必需/可選依賴。

```json
// Example: plugin manifest with dependencies
{
...
- **EN:** Provide a TypeScript/ESM template with strict interface, i18n, and test stubs. Encourage use of abstract base classes for common logic.
- **中文：** 提供 TypeScript/ESM 範本，嚴格型別、國際化與測試範例，建議共用邏輯抽象基底類別。

```typescript
// Example: Abstract base class for formatters
...
- **EN:** Require 100% test coverage for all plugin logic. Integrate static analysis (ESLint, TypeScript), security scan, and test reporting in CI/CD. Provide test fixtures and mocks for plugin APIs.
- **中文：** 強制所有插件邏輯 100% 測試覆蓋率，CI/CD 整合靜態分析、型別檢查、安全掃描與測試報告，並提供 API 測試樣板與 mock。

```typescript
// Example: Plugin test with mock API
import { mockApi } from '../test-utils';
test('formatter output', () => {
  const api = mockApi();
  const result = new SlackFormatter(api).format({ title: 'T', content: 'C' });
...
- **EN:** All user-facing strings must use i18n libraries (e.g., i18next) and support runtime language switching. Document supported locales in plugin manifest.
- **中文：** 所有對用戶顯示字串必須用 i18n 函式庫，支援執行期語言切換，並於描述檔標註支援語系。

```json
// Example: plugin manifest with locales
{
  "name": "@junai-key/formatter-slack",
  "locales": ["en", "zh-TW", "ja"]
}
...
- **EN:** Use advanced utility types (Partial, Required, Omit, ReturnType, Extract) and type guards for robust plugin contracts. Provide type-safe plugin registry and auto-completion.
- **中文：** 善用進階工具型別與 type guard，確保插件合約健壯，並提供型別安全的註冊表與自動補全。

```typescript
// Example: Type-safe plugin registry
const registry: Record<string, OutputFormatter> = {};
function registerFormatter(platform: string, formatter: OutputFormatter) {
  registry[platform] = formatter;
}
```

##### 7. Plugin Security & Sandbox Best Practices | 插件安全與沙箱最佳實踐

- **EN:** Always validate, sandbox, and restrict third-party plugins. Use process isolation, permission control, input/output validation, and audit logging. Document security model in plugin README.
- **中文：** 第三方插件必須驗證、沙箱隔離、權限控管、輸入/輸出驗證與稽核日誌，並於 README 詳述安全模型。

```typescript
// Example: Run plugin in a Node.js worker thread for isolation
import { Worker } from 'worker_threads';
function runPluginIsolated(pluginPath: string, input: any) {
  return new Promise((resolve, reject) => {
    const worker = new Worker(pluginPath, { workerData: input });
...
- **EN:** Each plugin must include a bilingual README (usage, API, i18n, test, version, security, changelog) and follow a clear, versioned contribution guide. Use badges for coverage, build, and security status.
- **中文：** 每個插件需附中英雙語 README（用法、API、國際化、測試、版本、安全、變更紀錄），並遵循明確版本化貢獻規範，建議加上覆蓋率、建置、安全徽章。
...
- **EN:** Monitor plugin execution time, memory, errors, and resource usage. Integrate with centralized logging, metrics, and tracing. Provide hooks for health checks and self-reporting.
- **中文：** 監控插件執行時間、記憶體、錯誤與資源用量，整合集中日誌、指標與追蹤，並提供健康檢查與自我回報鉤子。

```typescript
// Example: Plugin execution timing and health check
...
> For enterprise and open-source collaboration, always prioritize plugin security, documentation, observability, and internationalization. 持續強化插件安全、文件、可觀測性與國際化，讓國際協作與商業應用更順暢。
