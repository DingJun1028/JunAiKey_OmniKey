...
\
  useEffect(() => {\
    ...
    if (systemContext?.currentUser?.id) {\
        setCurrentUser(systemContext.currentUser); // Update currentUser state\
        fetchSettings(); // Fetch all settings on initial load\
    }\
\
    // --- New: Subscribe to realtime updates for profiles, sync_configs, linked_integrations ---\
    // Note: Supabase RLS on auth.users and profiles should ensure user only sees their own data.\
    // Sync configs and linked integrations would ideally be in user-specific tables.\
    // For MVP, we rely on SecurityService's in-memory state and refetching.\
    // A more robust approach would involve Realtime subscriptions to user-specific tables.\
\
    // Let's add listeners for events published by SecurityService for state changes\
    let unsubscribeAuth: (() => void) | undefined;\
    let unsubscribeSyncConfigLoaded: (() => void) | undefined;\
    let unsubscribeSyncConfigUpdated: (() => void) | undefined;\
    let unsubscribeIntegrationLinked: (() => void) | undefined;\
    let unsubscribeIntegrationLinkingFailed: (() => void) | undefined;\
    let unsubscribeIntegrationUnlinked: (() => void) | undefined;\
    let unsubscribeUserDataReset: (() => void) | undefined; // New: Subscribe to user data reset event\
    // --- New: Subscribe to backup events ---\
    let unsubscribeBackupStarted: (() => void) | undefined;\
    let unsubscribeBackupCompleted: (() => void) | undefined;\
    // --- End New ---\
    // --- New: Subscribe to integrity check events ---\
    let unsubscribeIntegrityCheckStarted: (() => void) | undefined;\
    let unsubscribeIntegrityCheckCompleted: (() => void) | undefined;\
    // --- End New ---\
    // --- New: Subscribe to security scan events ---\
    let unsubscribeSecurityScanStarted: (() => void) | undefined;\
    let unsubscribeSecurityScanCompleted: (() => void) | undefined;\
    // --- End New ---\
\
\
    if (systemContext?.eventBus) { // Check if EventBus is available\
        const eventBus = systemContext.eventBus;\
        const userId = systemContext?.currentUser?.id;\
\
        // Subscribe to auth state changes to update currentUser state\
        if (securityService) {\
             unsubscribeAuth = securityService.onAuthStateChange((user: User | null) => {\
                 setCurrentUser(user);\
                 // Fetch settings on login (handled by the main useEffect)\
                 // Clear state on logout (handled by the main useEffect else block)\
             });\
        }\
\
\
        // Subscribe to sync config events\
        unsubscribeSyncConfigLoaded = eventBus.subscribe('sync_config_loaded', (payload: { userId: string, config: CloudSyncConfig }) => {\
            if (payload.userId === userId) {\
                console.log('Settings page received sync_config_loaded event:', payload);\
                setSyncConfig(payload.config);\
            }\
        });\
         unsubscribeSyncConfigUpdated = eventBus.subscribe('sync_config_updated', (payload: { userId: string, config: CloudSyncConfig }) => {\
             if (payload.userId === userId) {\
                 console.log('Settings page received sync_config_updated event:', payload);\
                 setSyncConfig(payload.config);\
             }\
         });\
\
        // Subscribe to integration events\
        unsubscribeIntegrationLinked = eventBus.subscribe('integration_linked', (payload: { userId: string, integrationId: string }) => {\
             if (payload.userId === userId) {\
                 console.log('Settings page received integration_linked event:', payload);\
                 // Refetch linked integrations to update the list\
...
`);\
          ...
          unsubscribeIntegrationUnlinked = eventBus.subscribe('integration_unlinked', (payload: { userId: string, integrationId: string }) => {\
             if (payload.userId === userId) {\
                 console.log('Settings page received integration_unlinked event:', payload);\
                 // Refetch linked integrations to update the list\
                 securityService.getLinkedIntegrations(userId).then(setLinkedIntegrations).catch(err => console.error('Error refetching integrations:', err));\
                 alert(`;
Integration;
...
\
        // --- New: Subscribe to integrity check events ---\
        unsubscribeIntegrityCheckStarted = eventBus.subscribe('data_integrity_check_started', (payload: { userId: string }) => {\
            if (payload.userId === userId) {\
                console.log('Settings page received data_integrity_check_started event.');\
         ...
         unsubscribeIntegrityCheckCompleted = eventBus.subscribe('data_integrity_check_completed', (payload: { userId: string, results: any }) => {\
             if (payload.userId === userId) {\
                 console.log('Settings page received data_integrity_check_completed event:', payload);\
                 setIsCheckingIntegrity(false);\
                 setIntegrityCheckResult(payload.results);\
                 setIntegrityCheckError(payload.results?.status === 'error' ? payload.results.message : null);\
...
\
        // --- New: Subscribe to security scan events ---\
        unsubscribeSecurityScanStarted = eventBus.subscribe('security_scan_started', (payload: { userId: string }) => {\
            if (payload.userId === userId) {\
                console.log('Settings page received security_scan_started event.');\
         ...
         unsubscribeSecurityScanCompleted = eventBus.subscribe('security_scan_completed', (payload: { userId: string, results: any }) => {\
             if (payload.userId === userId) {\
                 console.log('Settings page received security_scan_completed event:', payload);\
                 setIsMonitoring(false);\
                 setSecurityScanResult(payload.results);\
                 setSecurityScanError(payload.results?.status === 'error' ? payload.results.message : null);\
                 setShowSecurityResultModal(true);\
             }\
         });\
        // --- End New ---\
    }\
...
\
    return () => {\
        ...
        unsubscribeAuth?.();\
        unsubscribeSyncConfigLoaded?.();\
        unsubscribeSyncConfigUpdated?.();\
        unsubscribeIntegrationLinked?.();\
        unsubscribeIntegrationLinkingFailed?.();\
        unsubscribeIntegrationUnlinked?.();\
        unsubscribeUserDataReset?.(); // Unsubscribe from reset event\
        // --- New: Unsubscribe from backup events ---\
        unsubscribeBackupStarted?.();\
        unsubscribeBackupCompleted?.();\
        // --- End New ---\
        // --- New: Unsubscribe from integrity check events ---\
        unsubscribeIntegrityCheckStarted?.();\
        unsubscribeIntegrityCheckCompleted?.();\
        // --- End New ---\
        // --- New: Unsubscribe from security scan events ---\
        unsubscribeSecurityScanStarted?.();\
        unsubscribeSecurityScanCompleted?.();\
        // --- End New ---\
    };\
\
  }, [systemContext?.currentUser?.id, securityService, syncService, notificationService, runeEngraftingCenter]); // Re-run effect when user ID or relevant services change\
\
\
    // --- New: Handle API Key Save ---\
    const handleSaveApiKeys = async (e: React.FormEvent) => {\
        ...
        try {\
            ...
            const newApiKeys = JSON.parse(JSON.stringify(apiKeys)); // Simple copy\
            // Call the service method to update API keys (Simulated)\
            // TODO: Implement updateApiKeys method in SecurityService\
            // await securityService.updateApiKeys(userId, newApiKeys); // Pass userId and new keys\
            console.log('Simulating saving API Keys:', newApiKeys);\
            await new Promise(resolve => setTimeout(resolve, 500)); // Simulate save time\
            setApiKeys(newApiKeys); // Update state with saved keys (simulated)\
\
             // Simulate recording user action (Part of \\u6b0a\\u80fd\\u935b\\u9020 / \\u516d\\u5f0f\\u5967\\u7fa9: \\u89c0\\u6e2c)\
            authorityForgingEngine?.recordAction({\
                type: 'web:settings:update_api_keys',\
                details: { updatedKeys: Object.keys(newApiKeys) },\
...
`);\
    ...
    // --- End New ---\
\
    // --- New: Handle Sync Config Save ---\
    const handleSaveSyncConfig = async (e: React.FormEvent) => {\
        e.preventDefault();\
        const userId = systemContext?.currentUser?.id;\
        if (!securityService || !userId || !syncConfig) {\
            alert(\\\"SecurityService module not initialized, user not logged in, or sync config not loaded.\\\");\
            return;\
        }\
        setIsSavingSyncConfig(true);\
        setSyncConfigError(null); // Clear specific error\
        try {\
            // Call the service method to update sync config (Simulated)\
            const updatedConfig = await securityService.updateSyncConfig(userId, syncConfig); // Pass userId and new config\
            if (updatedConfig) {\
                 alert('Sync Configuration updated successfully!');\
                 setIsEditingSyncConfig(false); // Close edit form\
                 // State update handled by realtime listener ('sync_config_updated' event)\
                  // Simulate recording user action\
                authorityForgingEngine?.recordAction({\
                    type: 'web:settings:update_sync_config',\
                    details: { config: updatedConfig },\
                    context: { platform: 'web', page: 'settings' },\
                    user_id: userId, // Associate action with user\
