...
if (!systemContext?.currentUser) {
    ...
    return (_jsx("div", { className: "container mx-auto p-4 flex justify-center", children: _jsx("div", { className: "bg-neutral-800/50 p-8 rounded-lg shadow-xl w-full max-w-md text-center text-neutral-300", children: _jsx("p", { children: "Please log in to view Agentic Flows." }) }) }));
}
return (_jsxs("div", { className: "container mx-auto p-4 flex flex-col h-[calc(100vh-80px)]", children: [" ", _jsxs("div", { className: "bg-neutral-800/50 p-6 rounded-lg shadow-xl flex flex-col flex-grow", children: [" ", loading ? (_jsx("p", { className: "text-neutral-400", children: "Loading flow details..." })) : error ? (_jsxs("p", { className: "text-red-400", children: ["Error: ", error] })) : !flow ? (_jsx("p", { className: "text-neutral-400", children: "Agentic Flow not found." })) : (_jsxs(_Fragment, { children: [_jsxs("div", { className: "flex items-center gap-4 mb-4", children: [_jsx(Link, { to: `/flows`, className: "text-neutral-400 hover:text-white transition", children: _jsx(ArrowLeft, { size: 24 }) }), _jsxs("h2", { className: "text-3xl font-bold text-blue-400", children: ["Flow: ", flow.name] })] }), _jsx("p", { className: "text-neutral-300 mb-4", children: flow.description || 'No description.' }), _jsxs("p", { className: "text-neutral-300 text-sm", children: ["Status: ", _jsx("span", { className: getFlowStatusColor(flow.status), children: flow.status })] }), _jsxs("small", { className: "text-neutral-400 text-xs block mb-4", children: ["ID: ", flow.id, " | Entry Node: ", flow.entry_node_id, flow.user_id && ` | Owner: ${flow.user_id}`, flow.creation_timestamp && ` | Created: ${new Date(flow.creation_timestamp).toLocaleString()}`, flow.start_timestamp && ` | Started: ${new Date(flow.start_timestamp).toLocaleString()}`, flow.completion_timestamp && ` | Finished: ${new Date(flow.completion_timestamp).toLocaleString()}`] }), _jsxs("div", { className: "mb-4 flex flex-wrap gap-2", children: [" ", (flow.status === 'idle' || flow.status === 'completed' || flow.status === 'failed' || flow.status === 'cancelled') && ((_jsxs("button", { className: "px-6 py-2 bg-green-600 text-white font-semibold rounded-md hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed", onClick: handleStartFlow, disabled: isStarting || isPausing || isResuming || isCancelling || isDeleting, children: [isStarting ? _jsx(Loader2, { size: 18, className: "inline-block mr-2 animate-spin" }) : _jsx(Play, { size: 18, className: "inline-block mr-2" }), ")}", isStarting ? 'Starting...' : 'Start'] }))), flow.status === 'in-progress' && ((_jsxs("button", { className: "px-6 py-2 bg-yellow-600 text-white rounded hover:bg-yellow-700 transition disabled:opacity-50 disabled:cursor-not-allowed", onClick: handlePauseFlow, disabled: isStarting || isPausing || isResuming || isCancelling || isDeleting, children: [isPausing ? _jsx(Loader2, { size: 18, className: "inline-block mr-2 animate-spin" }) : _jsx(Pause, { size: 18, className: "inline-block mr-2" }), ")}", isPausing ? 'Pausing...' : 'Pause'] }))), flow.status === 'paused' && ((_jsxs("button", { className: "px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed", onClick: handleResumeFlow, disabled: isStarting || isPausing || isResuming || isCancelling || isDeleting, children: [isResuming ? _jsx(Loader2, { size: 18, className: "inline-block mr-2 animate-spin" }) : _jsx(Play, { size: 18, className: "inline-block mr-2" }), ")}", isResuming ? 'Resuming...' : 'Resume'] }))), (flow.status === 'pending' || flow.status === 'in-progress' || flow.status === 'paused') && ((_jsxs("button", { className: "px-6 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition disabled:opacity-50 disabled:cursor-not-allowed", onClick: handleCancelFlow, disabled: isStarting || isPausing || isResuming || isCancelling || isDeleting, children: [isCancelling ? _jsx(Loader2, { size: 18, className: "inline-block mr-2 animate-spin" }) : _jsx(XCircle, { size: 18, className: "inline-block mr-2" }), ")}", isCancelling ? 'Cancelling...' : 'Cancel'] }))), (flow.status === 'failed' || flow.status === 'cancelled') && ((_jsxs("button", { className: "px-6 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed", onClick: () => handleStartFlow(), disabled: isStarting || isPausing || isResuming || isCancelling || isDeleting, children: [isStarting ? _jsx(Loader2, { size: 18, className: "inline-block mr-2 animate-spin" }) : _jsx(RotateCcw, { size: 18, className: "inline-block mr-2" }), ")} Retry"] }))), _jsxs(Link, { to: `/flows/${flow.id}/edit`, className: "px-6 py-2 text-sm bg-yellow-600 text-white font-semibold rounded-md hover:bg-yellow-700 transition disabled:opacity-50", children: [_jsx(Edit, { size: 18, className: "inline-block mr-2" }), " Edit Structure"] }), _jsxs("button", { className: "px-6 py-2 bg-red-800 text-white font-semibold rounded-md hover:bg-red-900 transition disabled:opacity-50 disabled:cursor-not-allowed", onClick: handleDeleteFlow, disabled: isStarting || isPausing || isResuming || isCancelling || isDeleting || flow.status === 'in-progress', children: [isDeleting ? _jsx(Loader2, { size: 18, className: "inline-block mr-2 animate-spin" }) : _jsx(Trash2, { size: 18, className: "inline-block mr-2" }), ")}", isDeleting ? 'Deleting...' : 'Delete'] })] }), _jsxs("div", { className: "flex-grow border border-neutral-600 rounded-md overflow-hidden", style: { height: '500px' }, children: [" ", _jsxs(ReactFlowProvider, { children: [" ", _jsxs(ReactFlow, { nodes: nodes, edges: edges, onNodesChange: onNodesChange, onEdgesChange: onEdgesState, nodeTypes: nodeTypes, fitView // Automatically fit the view to the nodes
                                            ...
                                            onNodeClick: onNodeClick, children: [_jsx(MiniMap, {}), _jsx(Controls, {}), _jsx(Background, { variant: "dots", gap: 12, size: 1 })] })] }), " "] }), selectedNodeDetails && (_jsxs("div", { className: "absolute top-4 right-4 bg-neutral-800/90 p-4 rounded-lg shadow-xl max-w-sm max-h-[95%] overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-700 z-10", children: [" ", _jsxs("div", { className: "flex justify-between items-center mb-3", children: [_jsx("h4", { className: "text-lg font-semibold text-blue-300", children: "Node Details" }), _jsx("button", { onClick: () => setSelectedNodeDetails(null), className: "text-neutral-400 hover:text-white", children: _jsx(XCircle, { size: 20 }) })] }), _jsxs("div", { className: "text-neutral-300 text-sm space-y-4", children: [" ", _jsxs("div", { children: [_jsxs("p", { children: [_jsx("span", { className: "font-semibold", children: "ID:" }), " ", selectedNodeDetails.node_id_in_flow] }), _jsxs("p", { children: [_jsx("span", { className: "font-semibold", children: "Type:" }), " ", selectedNodeDetails.type] }), _jsxs("p", { children: [_jsx("span", { className: "font-semibold", children: "Description:" }), " ", selectedNodeDetails.description] })] }), selectedNodeDetails.action && (_jsxs("div", { className: "p-3 bg-neutral-700/50 rounded-md", children: [" ", _jsx("h5", { className: "text-neutral-300 text-sm font-semibold mb-2", children: "Action Configuration:" }), _jsx(ActionEditor, { action: selectedNodeDetails.action, onChange: () => { }, disabled: true, availableRunes: availableRunes, availableAbilities: availableAbilities, availableGoals: availableGoals })] })), selectedNodeDetails.decision_logic && (_jsxs("div", { className: "p-3 bg-neutral-700/50 rounded-md", children: [" ", _jsx("h5", { className: "text-neutral-300 text-sm font-semibold mb-2", children: "Decision Logic (JSON):" }), _jsx("pre", { className: "bg-neutral-900 p-2 rounded-md text-neutral-200 text-xs font-mono overflow-x-auto max-h-24 scrollbar-thin scrollbar-thumb-neutral-700", children: JSON.stringify(selectedNodeDetails.decision_logic, null, 2) })] }))] })] })), _jsxs("div", { className: "mt-8 p-4 bg-neutral-700/50 rounded-lg", children: [_jsxs("h3", { className: "text-xl font-semibold text-blue-300 mb-3", children: ["Execution History (", executions.length, ")"] }), executions.length === 0 ? (_jsx("p", { className: "text-neutral-400", children: "No execution history found for this flow." })) : (_jsx("ul", { className: "space-y-4", children: executions.map((execution) => (_jsxs("li", { className: `bg-neutral-600/50 p-4 rounded-md border-l-4 ${getFlowStatusColor(execution.status).replace('text-', 'border-')}`, children: [_jsxs("div", { className: "flex justify-between items-center", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx(Workflow, { size: 20, className: "text-blue-400" }), _jsxs("h4", { className: `font-semibold mb-1 ${getFlowStatusColor(execution.status)}`, children: ["Execution ID: ", execution.id.substring(0, 8), "..."] })] }), _jsx("button", { onClick: () => toggleExpandExecution(execution.id), className: "text-neutral-400 hover:text-white transition", children: expandedExecutions[execution.id] ? _jsx(ChevronUp, { size: 20 }) : _jsx(ChevronDown, { size: 20 }) })] }), _jsxs("p", { className: "text-neutral-300 text-sm", children: ["Status: ", _jsx("span", { className: getFlowStatusColor(execution.status), children: execution.status })] }), _jsxs("small", { className: "text-neutral-400 text-xs block mt-1", children: ["Started: ", new Date(execution.start_timestamp).toLocaleString(), execution.completion_timestamp && ` | Finished: ${new Date(execution.completion_timestamp).toLocaleString()}`, execution.current_node_id && execution.status === 'in-progress' && ` | Current Node: ${execution.current_node_id}`] }), expandedExecutions[execution.id] && ((_jsxs("div", { className: "mt-4 border-t border-neutral-600 pt-4", children: [execution.error && (_jsxs("div", { className: "mb-4", children: [_jsx("h5", { className: "text-red-400 text-sm font-semibold mb-2", children: "Error:" }), _jsx("p", { className: "text-red-300 text-sm", children: execution.error })] })), execution.result && (_jsxs("div", { className: "mb-4", children: [_jsx("h5", { className: "text-neutral-300 text-sm font-semibold mb-2", children: "Result:" }), _jsx("pre", { className: "bg-neutral-800 p-3 rounded-md text-neutral-200 text-xs font-mono overflow-x-auto max-h-24 scrollbar-thin scrollbar-thumb-neutral-600", children: JSON.stringify(execution.result, null, 2) })] })), execution.execution_log_summary && (_jsxs("div", { className: "mb-4", children: [_jsx("h5", { className: "text-neutral-300 text-sm font-semibold mb-2", children: "Log Summary:" }), _jsx("pre", { className: "bg-neutral-800 p-3 rounded-md text-neutral-200 text-xs font-mono overflow-x-auto max-h-48 scrollbar-thin scrollbar-thumb-neutral-600", children: execution.execution_log_summary })] }))] })))] }, execution.id))) }))] })] }))] })] }));
;
