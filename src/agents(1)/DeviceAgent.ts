...
export class DeviceAgent extends BaseAgent {
    ...
    protected async handleMessage(message: AgentMessage): Promise<AgentResponse> {\\\
        console.log(`[DeviceAgent] Handling message: ${message.type} (Correlation ID: ${message.correlationId || 'N/A'})`);\\\
\
...
\
                case 'write_file': // Example using FileService\\\
                    ...
                    return { success: true, data: { message: `File written: ${message.payload.filePath}` } };\\\
\
                case 'delete_file': // Example using FileService\\\
                    ...
                    return { success: true, data: { message: `File deleted: ${message.payload.filePath}` } };\\\
\
...
\
                case 'merge_and_write_file': // Example using FileService\\\
                    ...
                    return { success: true, data: { message: `File merged and written: ${message.payload.filePath}` } };\\\
\
\
                ...
                case 'read_url':\\\
                    ...
                    const savedRecord = await this.context.knowledgeSync.saveKnowledge(\\\n                        `Analysis of URL: ${webResult.data.url}`, // Question\\\n                        `Summary: ${analysisResult.summary}\\\n\\\nKeywords: ${analysisResult.keywords.join(', ')}\\\n\\\nStructured Data: ${JSON.stringify(analysisResult.structuredData || {}, null, 2)}`, // Answer\\\n                        userId,\\\n                        'web-analysis', // Source\\\n                        { url: webResult.data.url, analysis: analysisResult } // Dev log details\\\
                    );\\\
                    if (!savedRecord) throw new Error('Failed to save URL analysis to knowledge base.');\\\
                    return { success: true, data: { message: `Analysis of ${webResult.data.url} saved to knowledge base.`, recordId: savedRecord.id, analysis: analysisResult } };\\\
\
\
                case 'analyze_file_content':\\\
                    ...
                    // Now, delegate saving the analysis result as a knowledge record.\\\
                    if (!this.context.knowledgeSync) throw new Error('KnowledgeSync is not available for saving analysis.');\\\
                    const savedFileRecord = await this.context.knowledgeSync.saveKnowledge(\\\n                        `Analysis of File: ${message.payload.fileMetadata?.name || 'Unknown File'}`, // Question\\\n                        `Summary: ${fileAnalysisResult.summary}\\\n\\\nKeywords: ${fileAnalysisResult.keywords.join(', ')}\\\n\\\nStructured Data: ${JSON.stringify(fileAnalysisResult.structuredData || {}, null, 2)}`, // Answer\\\
                        userId,\\\
                    ...
                    return { success: true, data: { message: `Analysis of ${message.payload.fileMetadata?.name || 'file'} saved to knowledge base.`, recordId: savedFileRecord.id, analysis: fileAnalysisResult } };\\\
\
                case 'simulate_visual_reading':\\\
                    ...
                    if (!this.context.knowledgeSync) throw new Error('KnowledgeSync is not available for saving analysis.');\\\
                    const savedVisualRecord = await this.context.knowledgeSync.saveKnowledge(\\\n                        `Text extracted from image`, // Question\\\n                        `Extracted Text: ${extractionResult.extractedText || 'N/A'}\\\n\\\nStructured Data: ${JSON.stringify({ calendarEvents: extractionResult.calendarEvents, tasks: extractionResult.tasks, contacts: extractionResult.contacts }, null, 2)}`, // Answer\\\
                        userId,\\\
                    ...
                    return { success: true, data: { message: `Text extracted from image and saved to knowledge base.`, recordId: savedVisualRecord.id, extraction: extractionResult } };\\\
\
                // --- End New ---\\\
\
\
                case 'handle_url_scheme':\\\
                    ...
                    console.log(`[DeviceAgent] URL Scheme received: ${message.payload.url}`);\\\
                    // For MVP, just log and return success\\\
                    return { success: true, data: { message: `Handled URL Scheme: ${message.payload.url}` } };\\\
\
\
                default:\\\
                    console.warn(`[DeviceAgent] Unknown message type: ${message.type}`);\\\
                    return { success: false, error: `Unknown message type for DeviceAgent: ${message.type}` };\\\
            }\\\n        } catch (error: any) {\\\
            console.error(`[DeviceAgent] Error handling message ${message.type}:`, error);\\\
