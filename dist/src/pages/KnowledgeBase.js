...
knowledgeRecords.length === 0 ? (_jsx("p", { className: true })) : ;
"text-neutral-400\">{searchTerm ? 'No records found matching your search.' : 'No records in the knowledge base yet.'}</p>;
(_jsx("ul", { className: true }));
"space-y-4\">;
{
    knowledgeRecords.map((record) => (_jsx("li", { className: true }, record.id)), "bg-neutral-600/50 p-4 rounded-md border-l-4 border-blue-500\">
...
{
    expandedRecords[record.id] ? _jsx(ChevronUp, { size: 20 }) : _jsx(ChevronDown, { size: 20 });
}
button >
;
div >
    ...
    { /* New: Relations Display */};
{
    ...
    "space-y-2\">;
    {
        record.relations.map(relation => (_jsx("li", { className: true }, relation.id)), ("p-2 bg-neutral-700/50 rounded-md text-neutral-300 text-xs flex justify-between items-center\">), { relation, : .source_record_id === record.id ? '->' : '<-' }, { relation, : .relation_type }, { relation, : .source_record_id === record.id ? 'to' : 'from' }, { relation, : .source_record_id === record.id ? relation.target_record_id : relation.source_record_id }, span >
            { /* Delete Relation Button */}, _jsx("button", { className: true }), "px-2 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700 transition disabled:opacity-50 disabled:cursor-not-allowed\", onClick = {}(), handleDeleteRelation(relation.id));
    }
    disabled = { isSavingRelation } // Disable while saving any relation
        >
            _jsx(Trash2, { size: 12, className: true });
...
disabled = { isSaving };
    >
        _jsx(Edit, { size: 16, className: true });
"inline-block mr-1\"/> Edit;
button >
    _jsx("button", { className: true });
...
disabled = { isSaving };
    >
        _jsx(Trash2, { size: 16, className: true });
"inline-block mr-1\"/> Delete;
button >
...
{
    availableRecordsForRelation.map(record => (_jsxs("option", { value: record.id, children: [record.question.substring(0, 50), "... (", record.id.substring(0, 8), "...)"] }, record.id)));
}
select >
;
div >
...
required
    >
        { RELATION_TYPES, : .map(type => (_jsx("option", { value: type, children: type.replace(/_/g, ' ') }, type))) };
select >
;
div >
...
disabled = { isSavingRelation } || !selectedSourceRecordId || !selectedTargetRecordId || !newRelationType || selectedSourceRecordId === selectedTargetRecordId;
    >
