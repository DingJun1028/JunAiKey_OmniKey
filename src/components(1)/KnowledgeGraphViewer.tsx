...
const GraphNode: React.FC<{ data: { label: string, knowledgeRecord: KnowledgeRecord } }> = ({ data }) => {
    ...
    return (
        <div className={`p-2 rounded-md border-l-4 ${borderColor} bg-neutral-700/70 shadow-md w-[${nodeWidth}px] h-[${nodeHeight}px] overflow-hidden text-ellipsis whitespace-nowrap flex flex-col`}>
...
const KnowledgeGraphViewer: React.FC<KnowledgeGraphViewerProps> = ({ userId, sourceFilter }) => { // Accept sourceFilter prop
  ...
  const fetchGraphData = useCallback(async () => {
      ...
      try {
          ...
          setError(`Failed to load knowledge graph: ${err.message}`);
      } finally {
          setLoading(false);
      }
  ...
  if (nodes.length === 0) {
      return <div className="text-neutral-400 text-center">No knowledge records found to build the graph{sourceFilter ? ` for source \"${sourceFilter}\"` : ''}.</div>; // Added filter info
  }
