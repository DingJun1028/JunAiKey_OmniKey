...
- **Plugin-based OutputFormatterFactory** / **插件化輸出格式工廠**：Easily add new output formats (e.g., Slack, CLI, Webhook, Mobile) by implementing a new formatter and registering it in the factory. 只需實作新 formatter 並註冊，即可支援 Slack、CLI、Webhook、行動端等多種格式。
- **Middleware Support** / **中間件支援**：Express middleware is pluggable, supporting authentication, logging, rate limiting, CORS, etc. Express 中間件可插拔，支援認證、日誌、速率限制、跨域等。
- **Agent Injection** / **代理注入**：Inject different agent or memory instances based on platform or request content. 可根據不同平台或請求內容注入不同的 agent 或 memory 實例。
- **API Versioning** / **API 版本管理**：Path design supports /v1/, /v2/, etc., for smooth upgrades. 路徑設計支援多版本共存，便於平滑升級。

> **Design Principle | 設計原則**
>
> - All extension points (formatters, agents, middleware) follow the Open/Closed Principle (OCP): open for extension, closed for modification. 所有擴展點（格式工廠、代理、中間件）皆遵循開放封閉原則，可擴展但不需修改核心。
> - Plugin registration and auto-discovery are recommended for large-scale collaboration and multi-platform scenarios. 插件註冊與自動發現適用於大型協作或多平台場景。
> - TypeScript interface/type definitions are strict to ensure extensibility safety. TypeScript 介面/型別定義嚴格，確保擴展安全。
...
// Example: Registering a new formatter plugin (插件註冊範例)
type TeamsFormatter = any; // class TeamsFormatter implements OutputFormatter
OutputFormatterFactory.register('teams', TeamsFormatter);

// Example: Auto-discover and register plugins (自動發現與註冊)
// plugins/index.ts
import { OutputFormatterFactory } from './OutputFormatterFactory';
import { formatters } from './all-formatters';
for (const [platform, Formatter] of Object.entries(formatters)) {
  OutputFormatterFactory.register(platform, Formatter);
}
```

> **Extensibility Best Practice | 擴展最佳實踐**
>
> - All extension points should be documented and exposed via TypeScript types/interfaces. 所有擴展點應有明確型別與文件。
> - Plugins should be discoverable and testable independently. 插件應可自動發現、獨立測試。
> - Plugin registration is best centralized in the startup flow or a dedicated registry file for maintainability. 插件註冊建議集中於啟動流程或專用 registry 檔案，便於維護。
> - For international teams, always provide bilingual comments and documentation. 國際協作建議雙語註解與說明。
