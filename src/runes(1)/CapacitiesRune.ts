...
export class CapacitiesRune {
    ...
    constructor(context: SystemContext, rune: Rune) {
        ...
        this.rune = rune;
        console.log(`[Rune] CapacitiesRune initialized for ${rune.name}.`);

        // Validate configuration (e.g., check if API key is available)
        if (!this.rune.configuration?.apiKey) {
             console.warn(`[Rune] CapacitiesRune ${rune.name} is missing API key in configuration.`);
             this.context.loggingService?.logWarning(`CapacitiesRune missing API key`, { runeId: rune.id });
        ...
        if (!this.rune.configuration?.userId) {
             console.warn(`[Rune] CapacitiesRune ${rune.name} is missing User ID in configuration.`);
             this.context.loggingService?.logWarning(`CapacitiesRune missing User ID`, { runeId: rune.id });
    ...
    async callAPI(params: { endpoint: string, method: string, data?: any, config?: any }, userId: string): Promise<any> {
        ...
        try {
            ...
            throw new Error(`Capacities API call failed: ${error.message}`);
        }
    }
    ...
    async getObjects(params?: { query?: string, limit?: number, type?: string }, userId?: string): Promise<any[]> {
        console.log(`[Rune] ${this.rune.name}.getObjects called by user ${userId} with params:`, params);
        this.context.loggingService?.logInfo(`Executing rune action: ${this.rune.name}.getObjects`, { runeId: this.rune.id, userId, params });
        ...
        console.log(`[Rune] Simulating getObjects successful. Found ${limitedObjects.length} objects.`);
    ...
    async searchObjects(params: { query: string, limit?: number, type?: string }, userId?: string): Promise<any[]> {
        console.log(`[Rune] ${this.rune.name}.searchObjects called by user ${userId} with params:`, params);
        this.context.loggingService?.logInfo(`Executing rune action: ${this.rune.name}.searchObjects`, { runeId: this.rune.id, userId, params });
