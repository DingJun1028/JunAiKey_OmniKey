...
export class SyncService {
    ...
    private async loadLocalChangeQueue(): Promise<void> {
        try {
            ...
            if (savedQueue) {
                ...
                console.log(`[SyncService] Loaded ${this.localChangeQueue.length} changes from persistence.`);
                this.context.loggingService?.logInfo(`Loaded ${this.localChangeQueue.length} changes from persistence.`);
                // Update status to reflect pending changes on startup
                if (this.localChangeQueue.length > 0) {
                     this.updateStatus('system', 'syncing', `Loaded ${this.localChangeQueue.length} pending changes`, this.context.currentUser?.id);
                }
            } else {
    ...
    private async saveLocalChangeQueue(): Promise<void> {
        try {
            ...
            // console.log(`[SyncService] Saved ${this.localChangeQueue.length} changes to persistence.`); // Avoid excessive logging
        } catch (error: any) {
    ...
    private updateStatus(dataType: SynchronizableDataType | 'system', status: 'idle' | 'syncing' | 'error' | 'unknown', step?: string, userId?: string): void {
        ...
        console.log(`[SyncService] Status update for ${dataType}: ${status} - ${step || 'N/A'} (Queue: ${this.localChangeQueue.length})`);
        this.context.loggingService?.logInfo(`Sync status update: ${dataType} - ${status}`, payload);
    ...
    async handleLocalDataChange(dataType: SynchronizableDataType, changeType: LocalDataChange['changeType'], payload: any, userId: string): Promise<void> {
        console.log(`[SyncService] Received local data change: ${dataType} - ${changeType} for user ${userId}`);
        this.context.loggingService?.logInfo(`Received local data change: ${dataType} - ${changeType}`, { dataType, changeType, userId, payload });
        ...
        const change: LocalDataChange = {
            id: `${dataType}-${changeType}-${Date.now()}-${Math.random().toString(16).slice(2)}`, // Unique ID for the change
            dataType,
            changeType,
        ...
        this.updateStatus('system', 'syncing', `Queued ${changeType} for ${dataType}`, userId); // Update system status
    ...
    private async processLocalChangeQueue(userId: string): Promise<void> {
        ...
        console.log(`[SyncService] Starting local change queue processing for user ${userId}. Queue size: ${this.localChangeQueue.length}`);
        ...
        try {
            while (this.localChangeQueue.length > 0) {
                ...
                if (pendingChanges.length === 0) {
                    ...
                    if (failedMaxRetries.length > 0) {
                         const errorMessage = `${failedMaxRetries.length} changes failed after ${MAX_RETRY_COUNT} retries.`;
                         this.updateStatus('system', 'error', `Sync failed: ${errorMessage}`, userId);
                ...
                console.log(`[SyncService] Processing batch of ${batchToProcess.length} changes.`);
                this.updateStatus('system', 'syncing', `Processing batch (${batchToProcess.length} items)`, userId);

                const pushPromises = batchToProcess.map(async (change) => {
                    this.updateStatus(change.dataType, 'syncing', `Pushing ${change.changeType} for ${change.dataType}`, userId); // Update status for specific type

                    try {
                        // Attempt to push the change to Supabase
                        const tables = DATA_TYPE_TABLE_MAP[change.dataType]; // Get tables for the data type
                        if (!tables || tables.length === 0) {
                            throw new Error(`Unknown table(s) for data type: ${change.dataType}`);
                        }
                        ...
                        if (dbError) {
                            ...
                            if (dbError.code === '23505' && change.changeType === 'INSERT') {
                                console.warn(`[SyncService] Conflict detected for change ${change.id} (Unique constraint violation).`);
                                this.context.loggingService?.logWarning(`Conflict detected for local change: ${change.id}`, { userId, change, error: dbError.message });
                                conflicts++;
                                ...
                                throw new Error(`Conflict: ${dbError.message}`); // Re-throw as a specific error type
                            }
                            // For other errors, re-throw to trigger retry/failure handling
                            throw dbError; // Re-throw the error
                        }

                        console.log(`[SyncService] Successfully pushed change ${change.id} to Supabase.`);
                        this.context.loggingService?.logInfo(`Successfully pushed local change: ${change.id}`, { userId, change });
                        itemsProcessed++;
                        ...
                        console.error(`[SyncService] Failed to push change ${change.id} for ${change.dataType}:`, error.message);
                        this.context.loggingService?.logError(`Failed to push local change for ${change.dataType}: ${change.id}`, { userId, change, error: error.message });
                        errors++;
                        ...
                        if (changeIndex > -1) {
                            ...
                            if (this.syncStatus[change.dataType] !== 'error') {
                                 this.updateStatus(change.dataType, 'error', `Push failed: ${error.message}`, userId);
                            }
                            this.updateStatus('system', 'syncing', `Push failed (${change.dataType}). Retrying...`, userId); // Keep system syncing/error

                             // Publish a sync_error event for this specific change
                             const errorPayload: SyncErrorPayload = { dataType: change.dataType, userId, error: error.message, timestamp: Date.now(), step: `Push failed for change ${change.id}`, change, retryCount: this.localChangeQueue[changeIndex].retryCount };
                             this.context.eventBus?.publish('sync_error', errorPayload, userId);
                        }
                ...
                if (remainingPending.length > 0) {
                    console.log(`[SyncService] ${remainingPending.length} changes remaining in queue. Retrying after delay.`);
                    this.updateStatus('system', 'syncing', `Retrying failed pushes...`, userId); // Keep system status as syncing
                    // Wait before processing the next batch/retrying
                    await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
                } else if (failedMaxRetries.length > 0) {
                     console.warn(`[SyncService] ${failedMaxRetries.length} changes failed after max retries.`);
                     this.context.loggingService?.logWarning(`${failedMaxRetries.length} changes failed after max retries.`, { userId, dataType: 'system', failedCount: failedMaxRetries.length });
                     const errorMessage = `${failedMaxRetries.length} changes failed after ${MAX_RETRY_COUNT} retries.`;
                     this.updateStatus('system', 'error', `Sync failed: ${errorMessage}`, userId); // Set system status to error
                     const errorPayload: SyncErrorPayload = { dataType: 'system', userId, error: errorMessage, timestamp: Date.now(), step: 'Max retries reached for some changes' };
                     this.context.eventBus?.publish('sync_error', errorPayload, userId);
            ...
            this.updateStatus('system', 'error', `Unexpected error processing queue: ${error.message}`, userId); // Set system status to error
    ...
    async syncAllData(userId: string): Promise<void> {
        ...
        try {
            ...
            console.log(`[SyncService] Full sync process initiated for user ${userId}. Final status will be reported by queue processor.`);

        } catch (error: any) {
            console.error(`[SyncService] Error during full sync for user ${userId}:`, error.message);
            this.context.loggingService?.logError(`Full sync failed for user ${userId}`, { userId, error: error.message });
            ...
            if (this.getSyncStatus('system') !== 'error') {
                 this.updateStatus('system', 'error', `Full sync failed: ${error.message}`, userId); // Update system status
    ...
    async syncDataType(dataType: SynchronizableDataType, userId: string, direction: 'up' | 'down' | 'bidirectional'): Promise<void> {
        console.log(`[SyncService] Initiating ${direction} sync for data type: ${dataType} for user: ${userId}...`);
        this.context.loggingService?.logInfo(`Initiating ${direction} sync for data type ${dataType}`, { dataType, userId, direction });
        ...
        if (this.getSyncStatus(dataType) === 'syncing') {
            console.warn(`[SyncService] ${dataType} sync already in progress. Skipping new sync request.`);
            this.context.loggingService?.logWarning(`${dataType} sync already in progress. Skipping new sync request.`, { dataType, userId });
            return;
        }

        this.updateStatus(dataType, 'syncing', `Starting ${direction} sync`, userId); // Update status for this type
        const startTime = Date.now();
        let itemsProcessed = 0;
        let conflicts = 0;
        let errors = 0;
        ...
        try {
            ...
            if (direction === 'down' || direction === 'bidirectional') {
                 ...
                 console.log(`[SyncService] Remote pull for ${dataType} initiated (relying on Realtime listeners).`);
            }
            ...
            if (direction === 'up' || direction === 'bidirectional') {
                 ...
                 if (changesForType.length > 0) {
                     this.updateStatus(dataType, 'syncing', `Pushing local changes (${changesForType.length} items)`, userId);
                     ...
                     while (processedCountInType < changesToProcess.length) {
                         ...
                         try {
                             ...
                             else { throw new Error(`Unknown change type: ${currentQueueChange.changeType}`); }
                             ...
                             this.context.loggingService?.logInfo(`Successfully pushed local change for ${dataType}: ${currentQueueChange.id}`, { userId, change: currentQueueChange });
                             itemsProcessed++;

                         } catch (error: any) {
                             console.error(`[SyncService] Failed to push change ${currentQueueChange.id} for ${currentQueueChange.dataType}:`, error.message);
                             this.context.loggingService?.logError(`Failed to push local change for ${currentQueueChange.dataType}: ${currentQueueChange.id}`, { userId, change: currentQueueChange, error: error.message });
                             errors++;
                             if (error.message.includes('Conflict')) conflicts++;
                             ...
                             if (mainQueueIdx > -1) {
                                 ...
                                 if (this.syncStatus[dataType] !== 'error') {
                                      this.updateStatus(dataType, 'error', `Push failed: ${error.message}`, userId);
                                 }
                             }
                     ...
                     console.log(`[SyncService] Finished processing local queue for data type: ${dataType}.`);
                 } else {
                     console.log(`[SyncService] No pending local changes for data type: ${dataType}.`);
                 }
            }
            ...
            if (pendingChangesForType.length === 0) {
                 this.updateStatus(dataType, 'idle', `${direction} sync completed`, userId); // Set status to idle
                 ...
                 this.context.eventBus?.publish('sync_completed', resultPayload, userId); // Publish completion event
                 console.log(`[SyncService] ${direction} sync completed for data type: ${dataType}.`);
            } else if (failedMaxRetriesForType.length > 0) {
                 console.warn(`[SyncService] ${direction} sync finished with failed changes for data type: ${dataType}. Failed: ${failedMaxRetriesForType.length}`);
                 this.context.loggingService?.logWarning(`${direction} sync finished with failed changes for ${dataType}`, { userId, dataType, failedCount: failedMaxRetriesForType.length });
                 const errorMessage = `${failedMaxRetriesForType.length} changes failed after ${MAX_RETRY_COUNT} retries.`;
                 this.updateStatus(dataType, 'error', `${direction} sync failed: ${errorMessage}`, userId); // Set status to error
                 const errorPayload: SyncErrorPayload = { dataType, userId, error: errorMessage, timestamp: Date.now(), step: `Sync failed for ${dataType} after retries`, direction };
                 this.context.eventBus?.publish('sync_error', errorPayload, userId);
                 ...
                 console.log(`[SyncService] ${direction} sync finished with pending changes (retries left) for data type: ${dataType}. Pending: ${pendingChangesForType.length}`);
                 this.updateStatus(dataType, 'syncing', `${direction} sync pending retries (${pendingChangesForType.length})`, userId); // Keep status as syncing
            ...
            console.error(`[SyncService] Error during ${direction} sync for data type ${dataType} for user ${userId}:`, error.message);
            this.context.loggingService?.logError(`Data type sync failed for ${dataType}`, { dataType, userId, direction, error: error.message });
            this.updateStatus(dataType, 'error', `${direction} sync failed: ${error.message}`, userId); // Set status to error
            this.updateStatus('system', 'error', `Sync failed for ${dataType}`, userId); // Update system status as well
            const errorPayload: SyncErrorPayload = { dataType, userId, error: error.message, timestamp: Date.now(), step: `Error during ${direction} sync`, direction };
            this.context.eventBus?.publish('sync_error', errorPayload, userId);
             // --- New: Save the queue state on unexpected error ---
             this.saveLocalChangeQueue().catch(err => console.error('Error saving queue on unexpected error:', err));
             // --- End New ---
    ...
    private async pullRemoteChanges(userId: string): Promise<void> {
        console.log(`[SyncService] Pulling remote changes for user: ${userId}...`);
        ...
        console.log(`[SyncService] Simulating fetching remote data for user: ${userId}.`);
        ...
        console.log(`[SyncService] Simulating merging remote data for user: ${userId}.`);
        ...
        console.log(`[SyncService] Remote change pull and merge process simulated.`);
    ...
    private async handleRemoteDataChange(table: string, eventType: 'INSERT' | 'UPDATE' | 'DELETE', newRecord: any, oldRecord: any, userId: string): Promise<void> {
        console.log(`[SyncService] Received remote data change via Realtime: ${table} - ${eventType} for user ${userId}`);
        this.context.loggingService?.logInfo(`Received remote data change: ${table} - ${eventType}`, { table, eventType, userId, newRecord, oldRecord });
        ...
        if (!dataType) {
            console.warn(`[SyncService] Received remote change for unknown table: ${table}. Skipping.`);
            this.context.loggingService?.logWarning(`Received remote change for unknown table: ${table}`, { table, eventType, userId });
            return;
        }

        this.updateStatus(dataType, 'syncing', `Applying remote ${eventType} for ${table}`, userId);
        ...
        try {
            ...
            console.log(`[SyncService] Remote change for ${table} processed (assuming handled by service's listener).`);
            this.updateStatus(dataType, 'idle', `Remote ${eventType} applied`, userId); // Update status for this data type
    ...
    */
     ...
     */
    async syncMobileGitRepo(userId: string, direction: 'pull' | 'push' | 'bidirectional', details?: any): Promise<any> {
        ...
        if (this.getSyncStatus('analyticsService') === 'syncing') { // Check the proxy status
        ...
        try {
            ...
            let resultDetails: any = { direction, ...details };
            let logMessage: string;
            let logType: 'sync-success' | 'sync-failure';
     ...
     * @returns Promise<void>
     */
    async backupAllData(userId: string, destination: 'google_drive' | 'local_file' = 'local_file'): Promise<void> {
     ...
     * @returns Promise<void>
     */
    async mirrorData(userId: string, dataType: SynchronizableDataType | 'system' = 'system', destination: string): Promise<void> {
    ...
    // TODO: Implement methods for configuring sync (e.g., frequency, wifi only).
    // TODO: Implement methods for viewing sync history/logs.
    // TODO: Implement conflict resolution strategies (e.g., LWW, CRDTs).
    // TODO: Implement background sync mechanisms (Web Workers, Service Workers).
    // TODO: Implement data export/import formats (JSON, CSV, Markdown).
    // TODO: This module is the core of the Bidirectional Sync Domain (\u96d9\u5410\u540c\u6b65\u9818\u57df).
}
```