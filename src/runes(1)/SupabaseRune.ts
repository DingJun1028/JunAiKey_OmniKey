...
export class SupabaseRune {
    ...
    constructor(context: SystemContext, rune: Rune) {
        ...
        if (!this.supabase) {
             ...
             this.context.loggingService?.logError(`SupabaseRune requires Supabase client`, { runeId: rune.id });
             // Throwing here might prevent the rune from being registered, which is appropriate.
             throw new Error('Supabase client is not available.');
        }

        console.log(`[Rune] SupabaseRune initialized for ${rune.name}.`);
    }
    ...
    async callRpc(params: { functionName: string, params?: any, useServiceRole?: boolean }, userId: string): Promise<any> {
        console.log(`[Rune] ${this.rune.name}.callRpc called by user ${userId} with params:`, params);
        this.context.loggingService?.logInfo(`Executing rune action: ${this.rune.name}.callRpc`, { runeId: this.rune.id, userId, params: { functionName: params?.functionName } });
        ...
        try {
            ...
            console.log(`[Rune] ${this.rune.name}.callRpc result:`, data);
            return { status: 'success', data: data };

        } catch (error: any) {
            console.error(`[Rune] Error executing ${this.rune.name}.callRpc:`, error);
            this.context.loggingService?.logError(`Error executing rune action: ${this.rune.name}.callRpc`, { runeId: this.rune.id, userId, error: error.message });
            throw new Error(`Supabase RPC call failed: ${error.message}`);
        }
    }
    ...
    async callRestApi(params: { endpoint: string, method: string, data?: any, config?: any, useServiceRole?: boolean }, userId: string): Promise<any> {
        console.log(`[Rune] ${this.rune.name}.callRestApi called by user ${userId} with params:`, params);
        this.context.loggingService?.logInfo(`Executing rune action: ${this.rune.name}.callRestApi`, { runeId: this.rune.id, userId, params: { endpoint: params?.endpoint, method: params?.method } });
        ...
        try {
            ...
            console.log(`[Rune] ${this.rune.name}.callRestApi result:`, result);
            return { status: 'success', data: result };

        } catch (error: any) {
            console.error(`[Rune] Error executing ${this.rune.name}.callRestApi:`, error);
            this.context.loggingService?.logError(`Error executing rune action: ${this.rune.name}.callRestApi`, { runeId: this.rune.id, userId, error: error.message });
            throw new Error(`Supabase REST API call failed: ${error.message}`);
        }
    }
