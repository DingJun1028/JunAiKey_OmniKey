  ...
  useEffect(() => {
    ...
    const handleCreateTerm = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (createdTerm) {
                alert(`Glossary term \\\"${createdTerm.term}\\\" created successfully!`);
    ...
    const handleUpdateTerm = async (e: React.FormEvent) => {
        e.preventDefault();
        const userId = systemContext?.currentUser?.id;
        if (!glossaryService || !editingTerm || !userId) return; // Safety checks
   ...
   const handleDeleteTerm = async (termId: string) => {
       ...
       if (!confirm(`Are you sure you want to delete this glossary term? You can only delete your private terms.`)) return;
    ...
    const handleRelatedConceptsInputChange = (e: React.ChangeEvent<HTMLInputElement>, isEditing: boolean) => {
    ...
    const getRelatedConceptsInputString = (isEditing: boolean) => {
       ...
       return (
            <div className=\"container mx-auto p-4 flex justify-center\">
               <div className=\"bg-neutral-800/50 p-8 rounded-lg shadow-xl w-full max-w-md text-center text-neutral-300\">
                   <p>Please log in to view the glossary.</p>
               </div>
            </div>
       );
  }
  ...
  return (
    <div className=\"container mx-auto p-4\">
      <div className=\"bg-neutral-800/50 p-6 rounded-lg shadow-xl\">
        <h2 className=\"text-3xl font-bold text-blue-400 mb-6\">Glossary (\u5b57\u5eab)</h2>
        <p className=\"text-neutral-300 mb-8\">Manage terms and definitions for your Jun.Ai.Key system and personal knowledge.</p>
