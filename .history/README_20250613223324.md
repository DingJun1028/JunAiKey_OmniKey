...
> **Section Overview**
> 
> This section details the architecture, extensibility, and loop-prevention logic of the dual-core sync integration center and agent group. It provides clear diagrams, technical explanations, and TypeScript code samples in both English and Chinese, ensuring professional documentation and easy extensibility for future contributors.
...
> The dual-core sync integration center orchestrates agent groups to enable robust, extensible, and loop-safe bi-directional synchronization across platforms (e.g., Boostspace, Supabase, Notion, Capacities). Each agent specializes in a sync task, with built-in state/version checks to prevent infinite loops and data conflicts.
...
| Feature                | Description (EN)                                                                 | 說明（中文）                                      |
|------------------------|----------------------------------------------------------------------------------|---------------------------------------------------|
| Modular Agent Group    | Each agent is pluggable and can be extended for new platforms.                   | 模組化代理群，可擴充支援新平台                    |
| Sync Loop Prevention   | All syncs include source marking and version/state comparison.                   | 同步皆標記來源並比對版本/狀態                     |
| Centralized Orchestration | The center manages agent coordination, error handling, and field mapping.     | 集中協調代理、錯誤處理與欄位對應                  |
| Extensible Mapping     | Field mapping is YAML-driven for easy cross-platform adaptation.                 | 欄位對應採 YAML 配置，易於跨平台擴展              |
...
export class BoostspaceAgent implements SyncAgent {
  ...
  async sync(payload: any) {
    // Implement Boostspace sync logic here
  }
}
...
// SyncCoordinator 負責協調所有代理，確保同步流程一致且可擴展。
```
...
| Step | Logic (EN)                                                                 | 邏輯（中文）                                      |
|------|----------------------------------------------------------------------------|---------------------------------------------------|
| 1    | Compare `version` or `updated_at` before writing.                          | 寫入前比對 `version` 或 `updated_at` 欄位         |
| 2    | Mark source (e.g., `source: boostspace` or `source: supabase`).            | 寫入時標記來源（如 boostspace 或 supabase）       |
| 3    | Only propagate changed data; prevent redundant triggers.                   | 僅傳遞變更資料，防止重複觸發與無限循環            |
