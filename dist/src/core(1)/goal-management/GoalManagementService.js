...
successfully. `, { id: goalId, userId: userId });\\\\\\\\\\\n\\\\\\\\\\\n              return goal;\\\\\\\\\\\n\\\\\\\\\\\n        } catch (error: any) {\\\\\\\\\\\n            console.error(`;
Error;
fetching;
goal;
$;
...
reached;
target;
value($, { kr, : .current_value } / $, { kr, : .target_value }).Marking;
`);\\\
                this.context.loggingService?.logInfo(`;
KR;
reached;
...
reach;
target;
value($, { kr, : .current_value } / $, { kr, : .target_value });
or;
is;
already;
completed.No;
...
`, { krId, taskId, userId: userId, error: error.message });\\\
            throw error; // Re-throw the error\\\
        }\\\
    }\\\
\\\
...
`) // Channel filtered by user_id\\\
            .on('postgres_changes', { event: '*', schema: 'public', table: 'goals', filter: `;
user_id = eq.$;
{
    userId;
}
