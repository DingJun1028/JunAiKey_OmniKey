...
const Keyboard: React.FC = () => { // Changed component name to Keyboard and added FC type
  ...
  useEffect(() => {
      ...
      const handler = setTimeout(() => {
          console.log(`Keyboard input changed, sending for suggestions: \"${input}\"`);
  ...
  // --- Modified: Handle Function Button Click (Refactored) ---
    ...
    const handleImageSelectForExtraction = async (event: React.ChangeEvent<HTMLInputElement>) => {
        const userId = systemContext?.currentUser?.id;
    ...
    const handleFileSelectForAnalysis = async (event: React.ChangeEvent<HTMLInputElement>) => {
        const userId = systemContext?.currentUser?.id;
        if (!userId || !systemContext?.messageBus || !event.target.files || !event.target.files[0]) {
    ...
    <div className="container mx-auto p-4 flex flex-col items-center">
      ...
      <section className="bg-white/95 dark:bg-neutral-900 rounded-3xl shadow-2xl pt-4 pb-7 px-3 w-full max-w-lg mb-3 border border-neutral-200 dark:border-neutral-800 select-none">
        ...
        <div className="flex justify-between items-center pb-3 px-2">
          <button onClick={()=>setKb(kb==='zh'?'en':'zh')} className="px-3 py-1.5 rounded-xl bg-neutral-200/70 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-200 text-xs font-semibold shadow active:scale-95 transition disabled:opacity-50"
          disabled={isSending} // Disable language switch while sending
          >
            {kb==='zh'?'\u5207\u63db\u82f1\u6587':'\u5207\u63db\u6ce8\u97f3'}
          </button>
            ...
            {BOPOMOFO_KEYS.map((row, rowIndex) => (
              <div key={rowIndex} className="flex justify-center gap-1">
                {row.map((key, keyIndex) => (
                  <button
                    key={keyIndex}
                    className={`flex-1 h-10 rounded-md flex items-center justify-center text-lg font-medium ${key ? 'bg-neutral-300/70 dark:bg-neutral-700/70 text-neutral-900 dark:text-neutral-100 active:bg-neutral-400/70 dark:active:bg-neutral-600/70' : 'bg-transparent cursor-default'} disabled:opacity-50`}
                    onClick={() => handleKeyClick(key)}
                    disabled={!key || isSending}
                  >
                    {key
```