...
to;
current;
user(filtered, by, RLS, or, client - side). `);
                }
            })
...
subscription;
error `, { userId, table, error: err?.message });
                 }
            });
...
try {
    ...
    if (useSemanticSearch) {
        ...
        if (!this.context.agentFactory?.getAgent('wisdom')) {
            ...
            const embeddingResponse = await this.context.agentFactory.getAgent('wisdom').requestAgent('wisdom', // Target the WisdomAgent
...
try {
    ...
    const records = data.map((item) => item.knowledge_records);
...
Promise < GlossaryTerm[] > {
    ...
    try: {
        ...
        dbQuery = dbQuery.order('term', { ascending: true }), // Order by term alphabetically
        const: { data, error } = await dbQuery,
        if(error) { }, throw: error,
        const: terms = data,
...
this.glossarySubscription = this.supabase
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'glossary' }, async (payload) => {
    console.log('[GlossaryService] Realtime glossary change received:', payload);
    const term = payload.new; // New data for INSERT/UPDATE
    const oldTerm = payload.old; // Old data for UPDATE/DELETE
