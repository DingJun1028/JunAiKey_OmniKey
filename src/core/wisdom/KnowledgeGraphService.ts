...
export class KnowledgeGraphService {
    ...
    async createRelation(sourceRecordId: string, targetRecordId: string, relationType: string, userId: string, details?: any): Promise<KnowledgeRelation | null> {
        console.log(`[KnowledgeGraphService] Creating relation between ${sourceRecordId} and ${targetRecordId} (${relationType}) for user ${userId}`);
    ...
    async getRelations(userId: string, sourceRecordId?: string, targetRecordId?: string, relationType?: string): Promise<KnowledgeRelation[]> {
        console.log(`[KnowledgeGraphService] Retrieving relations for user: ${userId}, source: ${sourceRecordId || 'any'}, target: ${targetRecordId || 'any'}, type: ${relationType || 'any'}`);
        ...
        try {
            ...
            console.log(`[KnowledgeGraphService] Fetched ${relations.length} relations for user ${userId}.`);
            this.context.loggingService?.logInfo(`Fetched ${relations.length} relations successfully.`, { userId });
    ...
    async getRelationsForRecord(recordId: string, userId: string): Promise<KnowledgeRelation[]> {
        console.log(`[KnowledgeGraphService] Retrieving relations for record ${recordId} for user ${userId}`);
        this.context.loggingService?.logInfo(`Attempting to fetch relations for record ${recordId}`, { recordId, userId });
        ...
        try {
            ...
            const { data, error } = await this.supabase
                ...
                .select('*')
                .eq('user_id', userId) // Filter by user ID
                .or(`source_record_id.eq.${recordId},target_record_id.eq.${recordId}`); // Record is source OR target

            if (error) { throw error; }

            const relations = data as KnowledgeRelation[];
            console.log(`[KnowledgeGraphService] Fetched ${relations.length} relations for record ${recordId}.`);
            this.context.loggingService?.logInfo(`Fetched ${relations.length} relations for record ${recordId} successfully.`, { recordId, userId });

            return relations;

        } catch (error: any) {
            console.error(`[KnowledgeGraphService] Error fetching relations for record ${recordId} from Supabase:`, error);
            this.context.loggingService?.logError(`Failed to fetch relations for record ${recordId}`, { recordId, userId: userId, error: error.message });
            return [];
        }
    }
    ...
    async deleteRelation(relationId: string, userId: string): Promise<boolean> {
        console.log(`[KnowledgeGraphService] Deleting relation ${relationId} from Supabase for user ${userId}...`);
        this.context.loggingService?.logInfo(`Attempting to delete knowledge relation ${relationId}`, { id: relationId, userId });
        ...
        try {
            ...
            // Filter by ID and user_id to ensure ownership
              ...
              if (deleted) {
                  console.log(`Knowledge relation ${relationId} deleted from Supabase.`);
                  ...
                  this.context.loggingService?.logInfo(`Knowledge relation deleted successfully: ${relationId}`, { id: relationId, userId: userId });\
              } else {\
                  console.warn(`Knowledge relation ${relationId} not found for deletion or user mismatch.`);\
                  this.context.loggingService?.logWarning(`Knowledge relation not found for deletion or user mismatch: ${relationId}`, { id: relationId, userId });\
              }\
              return deleted;\
\
        } catch (error: any) {\
            console.error(`Failed to delete knowledge relation ${relationId}:`, error);\
            this.context.loggingService?.logError(`Failed to delete knowledge relation ${relationId}`, { id: relationId, userId: userId, error: error.message });\
...
\
    ...
    async getGraphData(userId: string): Promise<{ nodes: KnowledgeRecord[], edges: KnowledgeRelation[] }> {\
        console.log(`[KnowledgeGraphService] Retrieving graph data for user: ${userId}...`);\
...
\
        try {\
            ...
            console.log(`[KnowledgeGraphService] Fetched ${nodes.length} nodes.`);\
\
            // Fetch all relations for the user\
            const edges = await this.getRelations(userId) || [];\
            console.log(`[KnowledgeGraphService] Fetched ${edges.length} edges.`);\
\
            ...
            console.log(`[KnowledgeGraphService] Filtered edges to ${filteredEdges.length} connecting fetched nodes.`);\
\
            console.log(`[KnowledgeGraphService] Retrieved graph data for user: ${userId}.`);\
            this.context.loggingService?.logInfo(`Retrieved knowledge graph data for user ${userId}`, { userId, nodeCount: nodes.length, edgeCount: filteredEdges.length });\
\
            return { nodes, edges: filteredEdges };\
\
        } catch (error: any) {\
            console.error(`[KnowledgeGraphService] Error retrieving graph data for user ${userId}:`, error);\
            this.context.loggingService?.logError(`Failed to retrieve knowledge graph data for user ${userId}`, { userId: userId, error: error.message });\
            throw error; // Re-throw the error\
        }\
    }\
\
