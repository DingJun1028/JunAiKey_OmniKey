...
- **Middleware Support**: Express 中間件可插拔，支援認證、日誌、速率限制、跨域等。
- **Agent Injection**: 可根據不同平台或請求內容注入不同的 agent 或 memory 實例。
- **API Versioning**: 路徑設計支援 /v1/、/v2/ 等多版本共存，便於平滑升級。
...
class OutputFormatterFactory {
  static getFormatter(platform: string): OutputFormatter {
    switch (platform) {
      ...
      case 'web': return new WebFormatter();
