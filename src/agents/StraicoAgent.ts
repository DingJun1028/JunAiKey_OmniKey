...
export class StraicoAgent extends BaseAgent {
    ...
    protected async handleMessage(message: AgentMessage): Promise<AgentResponse> {
        console.log(`[StraicoAgent] Handling message: ${message.type} (Correlation ID: ${message.correlationId || 'N/A'})`);
        ...
        try {
            ...
            switch (message.type) {
                ...
                default:
                    console.warn(`[StraicoAgent] Unknown message type: ${message.type}`);
                    return { success: false, error: `Unknown message type for StraicoAgent: ${message.type}` };
            }
        } catch (error: any) {
            console.error(`[StraicoAgent] Error handling message ${message.type}:`, error);
