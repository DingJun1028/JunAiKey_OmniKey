...
\\\n\\\n        } catch (error: any) {\\\
            console.error(`[DecisionAgent] Error analyzing intent for message ${message.type} (Correlation ID: ${message.correlationId || 'N/A'}):`, error);\\\
            this.context.loggingService?.logError(`DecisionAgent error analyzing intent.`, { userId, input: text, error: error.message, correlationId: message.correlationId });\\\
