...
export const LLMClient = {
  generatePlan: async (input: { task: string; context: unknown; availableSkills: string[] }) => {
    // 模擬 LLM 回應
    return {
      steps: [
...
interface LLMPlanResponse {
  steps: { skillType: string; parameters: Record<string, unknown> }[];
}

export const PlanParser = {
  parse: (llmResponse: LLMPlanResponse): Plan => ({
