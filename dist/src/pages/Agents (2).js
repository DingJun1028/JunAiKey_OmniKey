...
const handleUninstallRune = async (userRuneId) => {
    const userId = systemContext?.currentUser?.id;
    // --- Modified: Use sacredRuneEngraver ---
    if (!sacredRuneEngraver || !userId) {
        alert("SacredRuneEngraver module not initialized or user not logged in.");
...
const handleEditRuneConfigClick = (rune) => {
...
const handleSaveRuneConfig = async (e) => {
    e.preventDefault();
    const userId = systemContext?.currentUser?.id;
    // --- Modified: Use sacredRuneEngraver ---
...
const handleCreateAbility = async (e) => {
    ...
    try {
        ...
        const newAbility = await authorityForgingEngine.manuallyForgeAbility(newAbilityName, newAbilityDescription, newAbilityScript, triggerObject, // Use the assembled trigger object
        userId // Pass user ID
        );
        if (newAbility) {
...
const handleUpdateAbility = async (e) => {
    e.preventDefault();
    const userId = systemContext?.currentUser?.id;
...
const handleDeleteAbility = async (abilityId) => {
    ...
    if (!confirm(`Are you sure you want to delete ability ${abilityId}?`))
        return;
    setError(null);
    try {
...
const handleExecuteRuneMethod = async (e) => {
    e.preventDefault();
    const userId = systemContext?.currentUser?.id;
...
const renderTriggerDetailsForm = (triggerType, triggerDetails, setTriggerDetails, disabled) => {
    switch (triggerType) {
        case 'manual':
            return _jsx("p", { className: "text-neutral-400 text-sm", children: "This ability is triggered manually." });
        case 'keyword':
            return (_jsxs("div", { className: "mt-2 p-2 bg-neutral-600/50 rounded-md", children: [_jsx("label", { htmlFor: "trigger-keyword-value", className: "block text-neutral-400 text-xs font-semibold mb-1", children: "Keyword:" }), _jsx("input", { id: "trigger-keyword-value", type: "text", className: "w-full p-1 rounded-md bg-neutral-700 text-white border border-neutral-600 focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm", value: triggerDetails?.value || '', onChange: (e) => setTriggerDetails({ ...triggerDetails, value: e.target.value }), placeholder: "e.g., run report", disabled: disabled, required: true })] }));
        case 'schedule':
            return (_jsxs("div", { className: "mt-2 p-2 bg-neutral-600/50 rounded-md", children: [_jsx("label", { htmlFor: "trigger-schedule-cron", className: "block text-neutral-400 text-xs font-semibold mb-1", children: "Cron Expression:" }), _jsx("input", { id: "trigger-schedule-cron", type: "text", className: "w-full p-1 rounded-md bg-neutral-700 text-white border border-neutral-600 focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm", value: triggerDetails?.cron || '', onChange: (e) => setTriggerDetails({ ...triggerDetails, cron: e.target.value }), placeholder: "e.g., 0 * * * *", disabled: disabled, required: true }), _jsx("small", { className: "text-neutral-400 text-xs block mt-1", children: "Standard cron format (e.g., `0 * * * *` for every hour)." })] }));
        case 'event':
            return (_jsxs("div", { className: "mt-2 p-2 bg-neutral-600/50 rounded-md", children: [_jsx("label", { htmlFor: "trigger-event-type", className: "block text-neutral-400 text-xs font-semibold mb-1", children: "Event Type:" }), _jsx("input", { id: "trigger-event-type", type: "text", className: "w-full p-1 rounded-md bg-neutral-700 text-white border border-neutral-600 focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm", value: triggerDetails?.eventType || '', onChange: (e) => setTriggerDetails({ ...triggerDetails, eventType: e.target.value }), placeholder: "e.g., task_completed", disabled: disabled, required: true }), _jsx("small", { className: "text-neutral-400 text-xs block mt-1", children: "Triggered when this system event occurs." })] }));
        // Add cases for other trigger types (e.g., 'webhook', 'location', 'time')
        default:
            return (_jsxs("div", { className: "mt-2 p-2 bg-neutral-600/50 rounded-md", children: [_jsx("label", { htmlFor: "trigger-details-json", className: "block text-neutral-400 text-xs font-semibold mb-1", children: "Trigger Details (JSON):" }), _jsx("textarea", { id: "trigger-details-json", className: "w-full p-1 rounded-md bg-neutral-700 text-white border border-neutral-600 focus:outline-none focus:ring-1 focus:ring-blue-500 font-mono text-xs", value: JSON.stringify(triggerDetails || {}, null, 2), onChange: (e) => {
                            try {
                        ...
                        }, rows: 3, disabled: disabled, required: true })] }));
    }
};
// --- End New ---
...
if (!systemContext?.currentUser) {
    ...
    return (_jsx("div", { className: "container mx-auto p-4 flex justify-center", children: _jsx("div", { className: "bg-neutral-800/50 p-8 rounded-lg shadow-xl w-full max-w-md text-center text-neutral-300", children: _jsx("p", { children: "Please log in to view your scripts and abilities." }) }) }));
}
return (_jsx("div", { className: "container mx-auto p-4", children: _jsxs("div", { className: "bg-neutral-800/50 p-6 rounded-lg shadow-xl", children: [_jsx("h2", { className: "text-3xl font-bold text-blue-400 mb-6", children: "Scripts & Abilities (\u6B0A\u80FD\u935B\u9020)" }), _jsx("p", { className: "text-neutral-300 mb-8", children: "Manage your automated scripts (Abilities) and review recent system activity." }), !editingAbility && (_jsxs("div", { className: "mb-8 p-4 bg-neutral-700/50 rounded-lg", children: [_jsx("h3", { className: "text-xl font-semibold text-blue-300 mb-3", children: "Forge New Ability" }), _jsxs("form", { onSubmit: handleCreateAbility, children: [_jsxs("div", { className: "mb-4", children: [_jsx("label", { htmlFor: "newAbilityName", className: "block text-neutral-300 text-sm font-semibold mb-2", children: "Name:" }), _jsx("input", { id: "newAbilityName", type: "text", className: "w-full p-2 rounded-md bg-neutral-800 text-white border border-neutral-600 focus:outline-none focus:ring-2 focus:ring-blue-500", value: newAbilityName, onChange: (e) => setNewAbilityName(e.target.value), placeholder: "Enter ability name", disabled: isSavingAbility, required: true })] }), _jsxs("div", { className: "mb-4", children: [_jsx("label", { htmlFor: "newAbilityDescription", className: "block text-neutral-300 text-sm font-semibold mb-2", children: "Description (Optional):" }), _jsx("input", { id: "newAbilityDescription", type: "text", className: "w-full p-2 rounded-md bg-neutral-800 text-white border border-neutral-600 focus:outline-none focus:ring-2 focus:ring-blue-500", value: newAbilityDescription, onChange: (e) => setNewAbilityDescription(e.target.value), placeholder: "Enter description", disabled: isSavingAbility })] }), _jsxs("div", { className: "mb-4", children: [_jsx("label", { htmlFor: "newAbilityScript", className: "block text-neutral-300 text-sm font-semibold mb-2", children: "Script (JavaScript/TypeScript):" }), _jsx("textarea", { id: "newAbilityScript", className: "w-full p-2 rounded-md bg-neutral-800 text-white border border-neutral-600 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-xs", value: newAbilityScript, onChange: (e) => setNewAbilityScript(e.target.value), placeholder: "// Write your script here\\nconsole.log('Hello from ability!');", rows: 8, disabled: isSavingAbility, required: true })] }), _jsxs("div", { className: "mb-4 p-3 bg-neutral-600/50 rounded-md", children: [_jsx("h4", { className: "text-neutral-300 text-sm font-semibold mb-2", children: "Trigger:" }), _jsxs("div", { className: "mb-2", children: [_jsx("label", { htmlFor: "newAbilityTriggerType", className: "block text-neutral-400 text-xs font-semibold mb-1", children: "Type:" }), _jsxs("select", { id: "newAbilityTriggerType", className: "w-full p-1 rounded-md bg-neutral-800 text-white border border-neutral-600 focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm", value: newAbilityTriggerType, onChange: (e) => { setNewAbilityTriggerType(e.target.value); setNewAbilityTriggerDetails({}); }, disabled: isSavingAbility, required: true, children: [_jsx("option", { value: "manual", children: "manual" }), _jsx("option", { value: "keyword", children: "keyword" }), _jsx("option", { value: "schedule", children: "schedule" }), _jsx("option", { value: "event", children: "event" })] })] }), renderTriggerDetailsForm(newAbilityTriggerType, newAbilityTriggerDetails, setNewAbilityTriggerDetails, isSavingAbility)] }), _jsx("button", { type: "submit", className: "px-6 py-2 bg-green-600 text-white font-semibold rounded-md hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed", disabled: isSavingAbility || !newAbilityName.trim() || !newAbilityScript.trim() || (newAbilityTriggerType === 'keyword' && !newAbilityTriggerDetails?.value?.trim()) || (newAbilityTriggerType === 'schedule' && !newAbilityTriggerDetails?.cron?.trim()) || (newAbilityTriggerType === 'event' && !newAbilityTriggerDetails?.eventType?.trim()), children: isSavingAbility ? 'Forging...' : 'Forge Ability' }), _jsx("button", { type: "button", onClick: handleCancelCreate, className: "px-6 py-2 bg-neutral-600 text-white font-semibold rounded-md hover:bg-neutral-700 transition", disabled: isSavingAbility, children: "Cancel" })] }), error && isSavingAbility === false && ( // Show create error only after it finishes
                    _jsxs("p", { className: "text-red-400 text-sm mt-4", children: ["Error: ", error] }))] })), editingAbility && ( // Show edit form if editing
            _jsxs("div", { className: "mb-8 p-4 bg-neutral-700/50 rounded-lg", children: [_jsxs("h3", { className: "text-xl font-semibold text-blue-300 mb-3", children: ["Edit Ability: ", editingAbility.name] }), _jsxs("form", { onSubmit: handleUpdateAbility, children: [_jsxs("div", { className: "mb-4", children: [_jsx("label", { htmlFor: "editingAbilityName", className: "block text-neutral-300 text-sm font-semibold mb-2", children: "Name:" }), _jsx("input", { id: "editingAbilityName", type: "text", className: "w-full p-2 rounded-md bg-neutral-800 text-white border border-neutral-600 focus:outline-none focus:ring-2 focus:ring-blue-500", value: editingAbilityName, onChange: (e) => setEditingAbilityName(e.target.value), placeholder: "Edit ability name", disabled: isUpdatingAbility, required: true })] }), _jsxs("div", { className: "mb-4", children: [_jsx("label", { htmlFor: "editingAbilityDescription", className: "block text-neutral-300 text-sm font-semibold mb-2", children: "Description (Optional):" }), _jsx("input", { id: "editingAbilityDescription", type: "text", className: "w-full p-2 rounded-md bg-neutral-800 text-white border border-neutral-600 focus:outline-none focus:ring-2 focus:ring-blue-500", value: editingAbilityDescription, onChange: (e) => setEditingAbilityDescription(e.target.value), placeholder: "Edit description", disabled: isUpdatingAbility })] }), _jsxs("div", { className: "mb-4", children: [_jsx("label", { htmlFor: "editingAbilityScript", className: "block text-neutral-300 text-sm font-semibold mb-2", children: "Script (JavaScript/TypeScript):" }), _jsx("textarea", { id: "editingAbilityScript", className: "w-full p-2 rounded-md bg-neutral-800 text-white border border-neutral-600 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-xs", value: editingAbilityScript, onChange: (e) => setEditingAbilityScript(e.target.value), rows: 8, disabled: isUpdatingAbility, required: true })] }), _jsxs("div", { className: "mb-4 p-3 bg-neutral-600/50 rounded-md", children: [_jsx("h4", { className: "text-neutral-300 text-sm font-semibold mb-2", children: "Trigger:" }), _jsxs("div", { className: "mb-2", children: [_jsx("label", { htmlFor: "editingAbilityTriggerType", className: "block text-neutral-400 text-xs font-semibold mb-1", children: "Type:" }), _jsxs("select", { id: "editingAbilityTriggerType", className: "w-full p-1 rounded-md bg-neutral-800 text-white border border-neutral-600 focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm", value: editingAbilityTriggerType, onChange: (e) => { setEditingAbilityTriggerType(e.target.value); setEditingAbilityTriggerDetails({}); }, disabled: isUpdatingAbility, required: true, children: [_jsx("option", { value: "manual", children: "manual" }), _jsx("option", { value: "keyword", children: "keyword" }), _jsx("option", { value: "schedule", children: "schedule" }), _jsx("option", { value: "event", children: "event" })] })] }), renderTriggerDetailsForm(editingAbilityTriggerType, editingAbilityTriggerDetails, setEditingAbilityTriggerDetails, isUpdatingAbility)] }), _jsxs("div", { className: "flex gap-4", children: [_jsx("button", { type: "submit", className: "px-6 py-2 bg-green-600 text-white font-semibold rounded-md hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed", disabled: isUpdatingAbility || !editingAbilityName.trim() || !editingAbilityScript.trim() || (editingAbilityTriggerType === 'keyword' && !editingAbilityTriggerDetails?.value?.trim()) || (editingAbilityTriggerType === 'schedule' && !editingAbilityTriggerDetails?.cron?.trim()) || (editingAbilityTriggerType === 'event' && !editingAbilityTriggerDetails?.eventType?.trim()), children: isUpdatingAbility ? 'Saving...' : 'Save Changes' }), _jsx("button", { type: "button", onClick: handleCancelEditAbility, className: "px-6 py-2 bg-neutral-600 text-white font-semibold rounded-md hover:bg-neutral-700 transition", disabled: isUpdatingAbility, children: "Cancel" })] }), error && isUpdatingAbility === false && ( // Show save error only after it finishes
                            _jsxs("p", { className: "text-red-400 text-sm mt-4", children: ["Error: ", error] }))] })] })), editingRuneConfig && (_jsx("div", { className: "fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50", children: _jsxs("div", { className: "bg-neutral-800 p-8 rounded-lg shadow-xl w-full max-w-md", children: [_jsxs("div", { className: "flex justify-between items-center mb-4", children: [_jsxs("h3", { className: "text-xl font-semibold text-blue-300 mb-4", children: ["Edit Configuration for ", editingRuneConfig.name] }), _jsx("button", { type: "button", onClick: handleCancelEditRuneConfig, className: "text-neutral-400 hover:text-white transition", disabled: isSavingRuneConfig, children: _jsx(XCircle, { size: 24 }) })] }), _jsxs("form", { onSubmit: handleSaveRuneConfig, children: [_jsxs("div", { className: "mb-4", children: [_jsx("label", { htmlFor: "rune-config-json", className: "block text-neutral-300 text-sm font-semibold mb-2", children: "Configuration (JSON):" }), _jsx("textarea", { id: "rune-config-json", className: `w-full p-2 rounded-md bg-neutral-900 text-white border ${runeConfigEditError ? 'border-red-500' : 'border-neutral-700'} focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-xs`, value: editingConfigJson, onChange: (e) => {
                                            ...
                                            }, rows: 10, disabled: isSavingRuneConfig, required: true })] }), runeConfigEditError && _jsxs("p", { className: "text-red-400 text-sm mb-4", children: ["Error: ", runeConfigEditError] }), _jsxs("div", { className: "flex gap-4 justify-end", children: [_jsx("button", { type: "button", onClick: handleCancelEditRuneConfig, className: "px-6 py-2 bg-neutral-600 text-white font-semibold rounded-md hover:bg-neutral-700 transition", disabled: isSavingRuneConfig, children: "Close" }), _jsx("button", { type: "submit", className: "px-6 py-2 bg-green-600 text-white font-semibold rounded-md hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed", disabled: isSavingRuneConfig || !!runeConfigEditError, children: isSavingRuneConfig ? 'Saving...' : 'Save Configuration' })] })] })] }) })), showResultModal && (_jsx("div", { className: "fixed inset-0 bg-black bg-opacity50 flex justify-center items-center z-50", children: _jsxs("div", { className: "bg-neutral-800 p-8 rounded-lg shadow-xl w-full max-w-md", children: [_jsx("h3", { className: "text-xl font-semibold text-blue-300 mb-4", children: "Ability Execution Result" }), executionResult ? (_jsx("pre", { className: "bg-neutral-900 p-4 rounded-md text-neutral-200 text-xs font-mono overflow-x-auto max-h-64 scrollbar-thin scrollbar-thumb-neutral-700", children: JSON.stringify(executionResult, null, 2) })) : (_jsx("p", { className: "text-neutral-400", children: "No result available." })), _jsx("div", { className: "flex justify-end mt-6", children: _jsx("button", { type: "button", onClick: () => { setShowResultModal(false); setExecutionResult(null); }, className: "px-6 py-2 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700 transition", children: "Close" }) })] }) })), showExecuteRuneModal && (_jsx("div", { className: "fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50", children: _jsxs("div", { className: "bg-neutral-800 p-8 rounded-lg shadow-xl w-full max-w-md", children: [_jsxs("div", { className: "flex justify-between items-center mb-4", children: [_jsxs("h3", { className: "text-xl font-semibold text-blue-300", children: ["Execute Method on ", showExecuteRuneModal.name] }), _jsx("button", { type: "button", onClick: () => { setShowExecuteRuneModal(null); setSelectedRuneMethod(''); setRuneMethodParamsJson('{}'); setRuneMethodParamsError(null); setRuneMethodExecutionResult(null); setRuneMethodExecutionError(null); }, className: "text-neutral-400 hover:text-white transition", disabled: isExecutingRuneMethod, children: _jsx(XCircle, { size: 24 }) })] }), _jsx("p", { className: "text-neutral-300 text-sm mb-4", children: "Select a method to execute and provide parameters." }), _jsxs("form", { onSubmit: handleExecuteRuneMethod, children: [_jsxs("div", { className: "mb-4", children: [_jsx("label", { htmlFor: "runeMethodSelect", className: "block text-neutral-300 text-sm font-semibold mb-2", children: "Method:" }), _jsxs("select", { id: "runeMethodSelect", className: "w-full p-2 rounded-md bg-neutral-800 text-white border border-neutral-600 focus:outline-none focus:ring-2 focus:ring-blue-500", value: selectedRuneMethod, onChange: (e) => { setSelectedRuneMethod(e.target.value); setRuneMethodParamsJson('{}'); setRuneMethodParamsError(null); }, disabled: isExecutingRuneMethod, required: true, children: [_jsx("option", { value: "", children: "-- Select Method --" }), showExecuteRuneModal.manifest?.methods && Object.keys(showExecuteRuneModal.manifest.methods).map(methodName => (_jsx("option", { value: methodName, children: methodName }, methodName)))] })] }), selectedRuneMethod && (_jsxs("div", { className: "mb-4", children: [_jsx("label", { htmlFor: "runeMethodParamsJson", className: "block text-neutral-300 text-sm font-semibold mb-2", children: "Parameters (JSON):" }), _jsx("textarea", { id: "runeMethodParamsJson", className: `w-full p-2 rounded-md bg-neutral-800 text-white border ${runeMethodParamsError ? 'border-red-500' : 'border-neutral-600'} focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-xs`, value: runeMethodParamsJson, onChange: (e) => {
                                            ...
                                            }, rows: 6, disabled: isExecutingRuneMethod, required: true }), runeMethodParamsError && _jsxs("p", { className: "text-red-400 text-sm mt-1", children: ["Error: ", runeMethodParamsError] })] })), runeMethodExecutionResult && (_jsxs("div", { className: "mb-4", children: [_jsx("h4", { className: "text-neutral-300 text-sm font-semibold mb-2", children: "Result:" }), _jsx("pre", { className: "bg-neutral-900 p-3 rounded-md text-neutral-200 text-xs font-mono overflow-x-auto max-h-48 scrollbar-thin scrollbar-thumb-neutral-700", children: JSON.stringify(runeMethodExecutionResult, null, 2) })] })), runeMethodExecutionError && (_jsxs("div", { className: "mb-4", children: [_jsx("h4", { className: "text-neutral-300 text-sm font-semibold mb-2 text-red-400", children: "Error:" }), _jsx("pre", { className: "bg-neutral-900 p-3 rounded-md text-red-200 text-xs font-mono overflow-x-auto max-h-48 scrollbar-thin scrollbar-thumb-neutral-700", children: runeMethodExecutionError })] })), _jsxs("div", { className: "flex gap-4 justify-end mt-4", children: [_jsx("button", { type: "button", onClick: () => { setShowExecuteRuneModal(null); setSelectedRuneMethod(''); setRuneMethodParamsJson('{}'); setRuneMethodParamsError(null); setRuneMethodExecutionResult(null); setRuneMethodExecutionError(null); }, className: "px-6 py-2 bg-neutral-600 text-white font-semibold rounded-md hover:bg-neutral-700 transition", disabled: isExecutingRuneMethod, children: "Close" }), _jsx("button", { type: "submit", className: "px-6 py-2 bg-green-600 text-white font-semibold rounded-md hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed", disabled: isExecutingRuneMethod || !selectedRuneMethod || !!runeMethodParamsError, children: isExecutingRuneMethod ? 'Executing...' : 'Execute' })] })] })] }) }))] }) }));
;
