...
abstract class BaseAgent {
     ...
     */
    init(): void {
        console.log(`[Agent] ${this.agentName} init completed.`);
    ...
    receiveMessage(message: AgentMessage): void {
        console.log(`[Agent] ${this.agentName} received message: ${message.type} (Correlation ID: ${message.correlationId || 'N/A'})`);
        this.context.loggingService?.logDebug(`Agent received message: ${this.agentName}`, { messageType: message.type, correlationId: message.correlationId, sender: message.sender });
    ...
    private async processQueue(): Promise<void> {
        ...
        console.log(`[Agent] ${this.agentName} starting queue processing. Queue size: ${this.messageQueue.length}`);
        this.context.loggingService?.logDebug(`Agent starting queue processing: ${this.agentName}`, { queueSize: this.messageQueue.length });

        while (this.messageQueue.length > 0) {
            // Process one message at a time to maintain order
            const message = this.messageQueue.shift()!; // Get the next message

            console.log(`[Agent] ${this.agentName} processing message: ${message.type} (Correlation ID: ${message.correlationId || 'N/A'})`);
            this.context.loggingService?.logDebug(`Agent processing message: ${this.agentName}`, { messageType: message.type, correlationId: message.correlationId });
            ...
            try {
                ...
                if (typeof response !== 'object' || response === null || typeof response.success !== 'boolean') {
                     console.error(`[Agent] ${this.agentName} handleMessage for ${message.type} returned invalid response format:`, response);
                     this.context.loggingService?.logError(`Agent handleMessage returned invalid response format: ${this.agentName}`, { messageType: message.type, correlationId: message.correlationId, response });
                     response = { success: false, error: `Agent ${this.agentName} returned invalid response format.` };
                }
                // --- End Best Practice ---
                ...
                if (message.correlationId) {
                    ...
                    console.log(`[Agent] ${this.agentName} finished processing message ${message.type} without correlationId. No response sent.`);
                    this.context.loggingService?.logDebug(`Agent finished processing message without correlationId: ${this.agentName}`, { messageType: message.type });
                }

            } catch (error: any) {
                console.error(`[Agent] ${this.agentName} failed to handle message ${message.type} (Correlation ID: ${message.correlationId || 'N/A'}):`, error);
                this.context.loggingService?.logError(`Agent failed to handle message: ${this.agentName}`, { messageType: message.type, correlationId: message.correlationId, error: error.message });
                ...
                if (message.correlationId) {
                    ...
                    // --- End Best Practice ---
                     ...
                     console.error(`[Agent] ${this.agentName} failed to process message ${message.type} without correlationId. Error logged.`);
                }
            }
        }
        this.processing = false;
        console.log(`[Agent] ${this.agentName} queue empty. Stopping processing.`);
        this.context.loggingService?.logDebug(`Agent queue empty: ${this.agentName}`);
    }
     ...
     * They should return an `AgentResponse` object indicating success or failure.
     * If an unhandled error occurs within `handleMessage`, it should be thrown,
     * and the `processQueue` method will catch it and send an error response.
    ...
    protected sendResponse(originalMessage: AgentMessage, response: AgentResponse): void {\
        ...
        console.log(`[Agent] ${this.agentName} sending response for Correlation ID: ${originalMessage.correlationId}`);\
        this.context.loggingService?.logDebug(`Agent sending response: ${this.agentName}`, { correlationId: originalMessage.correlationId, success: response.success });\
...
\
     ...
     * @param message The message to send. Should include `type` and `payload`.\
     *                Can include `recipient` for targeted messages.\
     */\
    protected sendMessage(message: AgentMessage): void {\
        ...
        console.log(`[Agent] ${this.agentName} sending message: ${message.type} (Recipient: ${message.recipient || 'Broadcast'}) (Correlation ID: ${message.correlationId || 'N/A'})`);\
        this.context.loggingService?.logDebug(`Agent sending message: ${this.agentName}`, { messageType: message.type, recipient: message.recipient, correlationId: message.correlationId });\
...
\
        ...
        const correlationId = `req-${this.agentName}-${recipient}-${type}-${Date.now()}-${Math.random().toString(16).slice(2)}`;\
        // Note: We don't modify the original message object here, as it's created internally.\
\
        console.log(`[Agent] ${this.agentName} sending request and waiting for response: ${type} to ${recipient} (Correlation ID: ${correlationId})`);\
        this.context.loggingService?.logDebug(`Agent sending request and waiting: ${this.agentName}`, { recipient, messageType: type, correlationId });\
\
\
        return new Promise<AgentResponse>((resolve, reject) => {\
            ...
            const timer = setTimeout(() => {\
                if (this.pendingRequests.has(correlationId)) {\
                    ...
                    console.error(`[MessageBus] Request timed out: ${type} to ${recipient} (Correlation ID: ${correlationId})`);\
                    // --- Best Practice: Reject with AgentError ---\
                    reject(new AgentError(`Request to agent ${recipient} timed out after ${timeout}ms.`));\
                    // --- End Best Practice ---\
                }\
            }, timeout);\
\
