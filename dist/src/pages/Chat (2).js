...
pending;
result;
for (correlationId; $; { message, : .correlationId } `);
                    }
                    // --- End New ---\
...
const handleSuggestedActionClick = async (decision) => {
...
const handleFeedbackClick = async (recordId, feedbackType) => {
...
_jsxs("div", { className: "container mx-auto p-4 flex flex-col items-center", children: [_jsxs("header", { className: "text-center mb-6", children: [_jsxs("h1", { className: "text-3xl font-bold text-neutral-800 dark:text-neutral-200", children: ["Jun.Ai.key (", _jsx("span", { className: "text-blue-400", children: "\\u842c\\u80fd\\u5143\\u9375" }), ")"] }), _jsxs("p", { className: "text-neutral-600 dark:text-neutral-400", children: ["\\u9375\\u76e4 (", _jsx("span", { className: "text-blue-400", children: "\\u842c\\u80fd\\u5143\\u9375" }), ") \\u00d7 \\u4fbf\\u7c3d \\u00d7 AI \\u00d7 \\u5167\\u5bb9\\u6574\\u5408 \\u00b7 ToMemo/Flexiboard \\u98a8\\u683cDemo"] })] }), _jsxs("div", { className: "w-full max-w-lg h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-300 dark:scrollbar-thumb-neutral-700 mb-4 p-4 bg-neutral-800/50 rounded-lg shadow-inner flex flex-col space-y-4", children: [loading ? (_jsx("p", { className: "text-neutral-400 text-center", children: "Loading conversation history..." })) : error ? (_jsxs("p", { className: "text-red-400 text-center", children: ["Error loading history: ", error] })) : messages.length === 0 ? (_jsx("p", { className: "text-neutral-400 text-center", children: "Start a conversation!" })) : (messages.map((msg, index) => (_jsx("div", { className: `flex ${msg.source === 'ui-temp-user' || msg.source === 'dev-conversation' && msg.answer === '' ? 'justify-end' : 'justify-start'}`, children: _jsxs("div", { className: `p-3 rounded-lg max-w-[80%] ${msg.source === 'ui-temp-user' || msg.source === 'dev-conversation' && msg.answer === '' ? 'bg-blue-600 text-white' : msg.source === 'system-error' ? 'bg-red-800/50 text-red-200' : 'bg-neutral-700 text-neutral-100'}`, children: [msg.source === 'ui-temp-user' || msg.source === 'dev-conversation' && msg.answer === '' ? () : , "\\", _jsx("p", { children: msg.question }), ") : (\\", _jsxs("div", { className: "prose prose-invert max-w-none", children: [" ", _jsx(ReactMarkdown, { children: msg.answer })] }), ")}", msg.image_url && _jsx("img", { src: msg.image_url, alt: "Attached image", className: "mt-2 max-h-48 rounded-md" }), msg.audio_url && _jsx("audio", { controls: true, src: msg.audio_url, className: "mt-2 w-full" }), msg.file_url && msg.file_metadata && (_jsxs("div", { className: "mt-2 p-2 bg-neutral-600 rounded-md text-neutral-200 text-sm flex items-center gap-2", children: [_jsx(Paperclip, { size: 16 }), _jsxs("span", { children: ["Attached File: ", msg.file_metadata.name, " (", Math.round(msg.file_metadata.size / 1024), " KB)"] })] })), msg.aiDecision && msg.source !== 'ui-temp-user' && ( // Only show decision for AI messages
                            _jsxs("div", { className: "mt-2 pt-2 border-t border-neutral-600 text-xs", children: [_jsxs("p", { className: "font-semibold text-blue-300", children: ["AI Decision: ", msg.aiDecision.action.replace(/_/g, ' ').toUpperCase()] }), msg.aiDecision.action !== 'answer_via_ai' && msg.aiDecision.action !== 'present_suggestion' && ( // Don't show button for simple answer or present_suggestion (handled by UI)
                                    _jsxs("button", { className: "mt-2 px-3 py-1 text-xs bg-blue-500 text-white rounded hover:bg-blue-600 transition disabled:opacity-50 disabled:cursor-not-allowed", onClick: () => handleSuggestedActionClick(msg.aiDecision), disabled: isSending, children: [_jsx(Play, { size: 14, className: "inline-block mr-1" }), " Execute Action"] })), msg.aiDecision.action === 'present_suggestion' && (), "\\", _jsxs("button", { className: "mt-2 px-3 py-1 text-xs bg-purple-500 text-white rounded hover:bg-purple-600 transition disabled:opacity-50 disabled:cursor-not-allowed", onClick: () => handleSuggestedActionClick(msg.aiDecision), disabled: isSending, children: [_jsx(Lightbulb, { size: 14, className: "inline-block mr-1" }), " View Suggestion"] }), ")}\\"] })), msg.source !== 'ui-temp-user' && msg.source !== 'system-error' && ( // Only show feedback for AI responses (not user messages or system errors)
                            _jsxs("div", { className: "mt-2 pt-2 border-t border-neutral-600 text-xs flex items-center gap-2", children: [_jsx("span", { className: "text-neutral-400", children: "Feedback:" }), _jsxs("button", { className: `flex items-center gap-1 px-2 py-0.5 rounded ${feedbackStatus[msg.id] === 'correct' ? 'bg-green-600' : 'bg-neutral-600'} text-white hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed`, onClick: () => handleFeedbackClick(msg.id, 'correct'), disabled: isSending || feedbackStatus[msg.id] !== null, children: [_jsx(ThumbsUp, { size: 14 }), " Correct"] }), _jsxs("button", { className: `flex items-center gap-1 px-2 py-0.5 rounded ${feedbackStatus[msg.id] === 'incorrect' ? 'bg-red-600' : 'bg-neutral-600'} text-white hover:bg-red-700 transition disabled:opacity-50 disabled:cursor-not-allowed`, onClick: () => handleFeedbackClick(msg.id, 'incorrect'), disabled: isSending || feedbackStatus[msg.id] !== null, children: [_jsx(ThumbsDown, { size: 14 }), " Incorrect"] })] })), msg.relevantKnowledge && msg.relevantKnowledge.length > 0 && (_jsxs("div", { className: "mt-2 pt-2 border-t border-neutral-600 text-xs", children: [_jsx("p", { className: "font-semibold text-blue-300", children: "Relevant Knowledge Used:" }), _jsx("ul", { className: "list-disc list-inside text-neutral-400", children: msg.relevantKnowledge.map(kb => (_jsxs("li", { className: "text-xs", children: ["Q: ", kb.question.substring(0, 50), "... (ID: ", kb.id.substring(0, 8), "...)"] }, kb.id))) })] }))] }) }, msg.id || index)))), _jsx("div", { ref: messagesEndRef }), " "] }), _jsx("nav", { className: "w-full max-w-lg flex flex-row items-center gap-2 py-3 px-1 overflow-x-auto scrollbar-thin scrollbar-thumb-neutral-200 dark:scrollbar-thumb-neutral-700 mb-1", children: FUNCTION_TABS.map(({ icon: Icon, label, actionType }) => ( // Added actionType
            _jsxs("button", { className: "flex flex-col items-center justify-center px-3 py-2 rounded-2xl bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-800 shadow active:scale-95 hover:bg-blue-50 hover:dark:bg-blue-900 transition flex-shrink-0 disabled:opacity-50 disabled:cursor-not-allowed", onClick: () => handleFunctionClick(label, actionType), disabled: isSending || !!processingFunctionLabel, children: [_jsx(Icon, { size: 24, className: "mb-0.5 text-blue-500" }), _jsx("span", { className: "text-xs text-neutral-600 dark:text-neutral-300 font-semibold", children: processingFunctionLabel === label ? '...' : label })] }, label))) }), _jsxs("div", { className: "w-full max-w-lg px-4 py-3 rounded-xl bg-white/90 shadow border border-neutral-100 dark:bg-neutral-900/90 dark:border-neutral-800 flex items-center justify-between select-none mb-2", children: [_jsx("div", { className: "text-lg tracking-widest font-mono text-neutral-900 dark:text-neutral-100 whitespace-nowrap overflow-x-auto max-w-[75vw] scrollbar-thin", children: input || _jsx("span", { className: "text-neutral-300 dark:text-neutral-600", children: "\\u8acb\\u8f38\\u5165\\u2026" }) }), _jsx("button", { className: "ml-2 px-2 py-1 text-xs rounded hover:bg-neutral-200 dark:hover:bg-neutral-800 transition text-neutral-600 dark:text-neutral-300", onClick: () => setInput(""), disabled: isSending, children: "\\u6e05\\u9664" })] }), _jsxs("section", { className: "w-full max-w-lg flex flex-col h-32 overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-300 dark:scrollbar-thumb-neutral-700 mb-2 gap-2", children: [PHRASES.map(section => ()), "\\", _jsxs("div", { className: "", children: [_jsx("div", { className: "text-xs text-neutral-500 dark:text-neutral-400 font-bold mb-1 pl-1", children: section.group }), _jsxs("div", { className: "flex flex-row flex-wrap gap-2", children: [section.items.map((phrase, i) => ()), "\\", _jsx("button", { className: "px-3 py-2 rounded-2xl bg-[#f7fafe] dark:bg-neutral-800 border border-neutral-100 dark:border-neutral-700 shadow-sm hover:bg-blue-100 hover:dark:bg-neutral-600 text-[15px] font-medium text-neutral-700 dark:text-neutral-50 flex-shrink-0 active:scale-95 transition disabled:opacity-50 disabled:cursor-not-allowed", onClick: () => handlePhraseClick(phrase), disabled: isSending, children: phrase }, phrase + i), "))\\ }\\"] })] }, section.group), "))\\ }\\"] }), _jsxs("div", { className: "w-full max-w-lg flex flex-col gap-2 mb-3 px-2", children: [_jsxs("div", { className: "flex flex-col gap-1", children: [_jsx("div", { className: "text-xs text-neutral-500 dark:text-neutral-400 font-bold mb-1 pl-1", children: "\\u667a\\u6167\\u63a8\\u85a6 (AI Suggestions)" }), _jsxs("div", { className: "flex flex-row flex-wrap gap-2 min-h-[30px]", children: [" ", isLoadingSuggestions ? () : , "\\", _jsx("span", { className: "text-neutral-400 text-sm", children: "\\u8f09\\u5165\\u5efa\\u8b70\\u4e2d..." }), ") : smartSuggestions.length > 0 ? (\\ smartSuggestions.map((suggestion, i) => (\\", _jsxs("button", { className: "px-3 py-2 rounded-2xl bg-blue-500/80 hover:bg-blue-600/80 text-white text-[15px] font-medium shadow-sm flex-shrink-0 active:scale-95 transition disabled:opacity-50 disabled:cursor-not-allowed", onClick: () => handleSuggestionClick(suggestion), disabled: isSending, children: [suggestion.action === 'answer_via_ai' ? suggestion.parameters?.question?.substring(0, 30) + '...' : suggestion.action.replace(/_/g, ' '), "\\"] }, 'ai-sug-' + i), "))\\ ) : (\\ !isLoadingSuggestions && ", _jsx("span", { className: "text-neutral-400 text-sm", children: "\\u7121\\u667a\\u6167\\u63a8\\u85a6" }), ")\\ }\\"] })] }), _jsxs("div", { className: "flex flex-col gap-1", children: [_jsxs("div", { className: "flex justify-between items-center mb-1 pl-1", children: [_jsx("div", { className: "text-xs text-neutral-500 dark:text-neutral-400 font-bold", children: "\\u77e5\\u8b58\\u5eab\\u63d2\\u5165 (Knowledge Base)" }), _jsxs("button", { className: `text-xs font-semibold px-2 py-0.5 rounded-full transition ${useSemanticSearch ? 'bg-blue-500 text-white' : 'bg-neutral-200 text-neutral-700 dark:bg-neutral-700 dark:text-neutral-300'} disabled:opacity-50 disabled:cursor-not-allowed`, onClick: () => setUseSemanticSearch(!useSemanticSearch), disabled: isLoadingKnowledge || isSending, children: [useSemanticSearch ? '\u8a9e\u7fa9\u641c\u7d22 ON' : '\u8a9e\u7fa9\u641c\u7d22 OFF', "\\"] })] }), _jsxs("div", { className: "flex flex-row flex-wrap gap-2 min-h-[30px]", children: [" ", isLoadingKnowledge ? () : , "\\", _jsx("span", { className: "text-neutral-400 text-sm", children: "\\u641c\\u5c0b\\u77e5\\u8b58\\u5eab\\u4e2d..." }), ") : knowledgeResults.length > 0 ? (\\ knowledgeResults.map((record) => (\\", _jsxs("button", { className: "px-3 py-2 rounded-2xl bg-green-400/80 hover:bg-green-500/80 text-white text-[15px] font-medium shadow-sm flex-shrink-0 active:scale-95 transition disabled:opacity-50 disabled:cursor-not-allowed", onClick: () => handleKnowledgeClick(record), title: `Q: ${record.question}`, disabled: isSending, children: [record.answer.substring(0, 30), record.answer.length > 30 ? '...' : '', " "] }, 'kb-res-' + record.id), "))\\ ) : (\\ !isLoadingKnowledge && ", _jsx("span", { className: "text-neutral-400 text-sm", children: "\\u7121\\u76f8\\u95dc\\u77e5\\u8b58" }), ")\\ }\\"] })] })] }), _jsxs("div", { className: "w-full max-w-lg flex flex-col gap-2 mb-3 px-2", children: [_jsx("div", { className: "text-xs text-neutral-500 dark:text-neutral-400 font-bold mb-1 pl-1", children: "Mobile Git Sync (\\u6a21\\u64ec)" }), _jsxs("div", { className: "flex flex-row flex-wrap gap-2", children: [MOBILE_GIT_SYNC_BUTTONS.map(({ icon: Icon, label, action }) => ()), "\\", _jsx("button", { className: "px-3 py-2 rounded-2xl bg-purple-500/80 hover:bg-purple-600/80 text-white text-[15px] font-medium shadow-sm flex-shrink-0 active:scale-95 transition disabled:opacity-50 disabled:cursor-not-allowed", onClick: () => handleMobileGitSyncClick(action) }, action), "\\ disabled=", isSending || !!processingFunctionLabel, " // Disable while sending or any function is processing >", _jsx(Icon, { size: 18, className: `inline-block mr-1 ${processingFunctionLabel === `Git ${action}` ? 'animate-pulse' : ''}` }), processingFunctionLabel === `Git ${action}` ? '...' : label, "\\"] }), "))\\ }\\"] })] })) /* --- End New --- */;
{ /* --- End New --- */ }
{ /* Input Area and Send Button */ }
_jsxs("form", { onSubmit: handleSend, className: "w-full max-w-lg flex items-center gap-2 px-2", children: [_jsx("input", { type: "file", accept: "image/*", onChange: handleImageSelect, style: { display: 'none' }, id: "imageInput" }), _jsx("label", { htmlFor: "imageInput", className: "p-2 rounded-md bg-neutral-700 text-neutral-300 hover:bg-neutral-600 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed", title: "Attach Image", children: _jsx(Image, { size: 20 }) }), _jsx("input", { type: "file", accept: "audio/*", onChange: handleAudioSelect, style: { display: 'none' }, id: "audioInput" }), _jsx("label", { htmlFor: "audioInput", className: "p-2 rounded-md bg-neutral-700 text-neutral-300 hover:bg-neutral-600 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed", title: "Attach Audio", children: _jsx(Mic, { size: 20 }) }), _jsx("input", { type: "file", onChange: handleFileSelect, style: { display: 'none' }, id: "fileInput" }), _jsx("label", { htmlFor: "fileInput", className: "p-2 rounded-md bg-neutral-700 text-neutral-300 hover:bg-neutral-600 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed", title: "Attach File", children: _jsx(Paperclip, { size: 20 }) }), _jsx("input", { type: "text", className: "flex-grow p-3 rounded-xl bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100 border border-neutral-300 dark:border-neutral-700 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50", value: input, onChange: (e) => setInput(e.target.value) }), "\\ placeholder=\"Chat with Jun.Ai.Key...\" disabled=", isSending, " // Disable input while sending />", _jsxs("button", { type: "submit", className: "p-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition disabled:opacity-50 disabled:cursor-not-allowed", disabled: isSending || (!input.trim() && !selectedImage && !selectedAudio && !selectedFile), children: [isSending ? _jsx(Loader2, { size: 24, className: "animate-spin" }) : _jsx(Send, { size: 24 }), "\\"] })] });
{ /* New: Display selected multimodal content */ }
{
    (selectedImage || selectedAudio || selectedFile) && ();
    _jsxs("div", { className: "w-full max-w-lg mt-2 p-3 bg-neutral-700/50 rounded-lg text-neutral-300 text-sm flex items-center justify-between", children: [_jsxs("div", { children: [selectedImage && _jsxs("p", { children: ["Image selected: ", selectedImage.name] }), "\\", selectedAudio && _jsxs("p", { children: ["Audio selected: ", selectedAudio.name] }), "\\", selectedFile && _jsxs("p", { children: ["File selected: ", selectedFile.name] }), "\\"] }), _jsxs("button", { onClick: handleClearMultimodal, className: "text-red-400 hover:text-red-600", children: [_jsx(XCircle, { size: 20 }), " Clear"] })] });
}
{ /* End New */ }
{ /* New: Display main error */ }
{
    error && ();
    _jsxs("div", { className: "w-full max-w-lg mt-4 p-3 bg-red-800/50 rounded-lg text-red-200 text-sm", children: ["Error: ", error] });
}
{ /* End New */ }
div >
;
