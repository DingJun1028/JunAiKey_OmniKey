...
import { BookKey, Target, Workflow, Zap, Code, FileText, GitCommit, Info, BookText } from 'lucide-react'; // Import icons, add BookText
    ...
    <div className="container mx-auto p-4">
      <div className="bg-neutral-800/50 p-6 rounded-lg shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {/* New: Sacred Codex Records (Developer Memoirs) Link */}
            <Link to="/sacred-codex-records" className="block p-6 bg-neutral-700/50 rounded-lg shadow hover:bg-neutral-600/50 transition">
                <div className="flex items-center gap-4 mb-4">
                    <BookText size={40} className="text-pink-400"/>
                    <h3 className="text-xl font-semibold text-pink-300">Developer Memoirs</h3>
                </div>
                <p className="text-neutral-300 text-sm">Review the raw, unfiltered development logs and system memoirs.</p>
            </Link>
            {/* --- End New --- */}
            ...
            {/* Boost.space 雙向同步 Link - 強化 UIUX，加入同步狀態與操作說明 */}
            <div className="relative block p-6 bg-neutral-700/50 rounded-lg shadow hover:bg-yellow-700/40 transition group">
                <div className="flex items-center gap-4 mb-4">
                    <Zap size={40} className="text-yellow-400 animate-pulse group-hover:animate-spin"/>
                    <h3 className="text-xl font-semibold text-yellow-300">Boost.space 雙向同步</h3>
                </div>
                <p className="text-neutral-300 text-sm mb-2">同步 Boost.space 任務、知識、專案，支援雙向自動更新與衝突解決。</p>
                <button className="mt-2 px-4 py-2 bg-yellow-500 text-neutral-900 rounded hover:bg-yellow-400 transition font-semibold shadow" onClick={() => alert('即將啟動 Boost.space 雙向同步！')}>立即同步</button>
                <div className="absolute top-2 right-4 text-xs text-yellow-300 opacity-80">API Ready</div>
                <a href="/api/boostspace-sync" className="block mt-2 text-xs text-yellow-200 underline hover:text-yellow-100">API 範例</a>
            </div>

            {/* Capacities 知識庫相容 Link - 強化 UIUX，加入匯入/匯出操作與說明 */}
            <div className="relative block p-6 bg-neutral-700/50 rounded-lg shadow hover:bg-indigo-700/40 transition group">
                <div className="flex items-center gap-4 mb-4">
                    <BookText size={40} className="text-indigo-400 group-hover:animate-bounce"/>
                    <h3 className="text-xl font-semibold text-indigo-300">Capacities 知識庫相容</h3>
                </div>
                <p className="text-neutral-300 text-sm mb-2">導入/匯出 Capacities 格式，與多平台知識庫無縫整合。</p>
                <div className="flex gap-2 mt-2">
                  <button className="px-4 py-2 bg-indigo-500 text-neutral-100 rounded hover:bg-indigo-400 transition font-semibold shadow" onClick={() => alert('匯入 Capacities JSON！')}>匯入</button>
                  <button className="px-4 py-2 bg-indigo-500 text-neutral-100 rounded hover:bg-indigo-400 transition font-semibold shadow" onClick={() => alert('匯出 Capacities JSON！')}>匯出</button>
                </div>
                <div className="absolute top-2 right-4 text-xs text-indigo-300 opacity-80">API Ready</div>
                <a href="/api/capacities-compat" className="block mt-2 text-xs text-indigo-200 underline hover:text-indigo-100">API 範例</a>
            </div>
