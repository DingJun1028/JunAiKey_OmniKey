...
- **EN:** Always validate and sandbox third-party plugins. Use process isolation, permission control, and input validation to prevent malicious code execution.
- **中文：** 第三方插件必須驗證與沙箱隔離，建議採用進程隔離、權限控管與輸入驗證，防止惡意程式碼執行。
...
- **EN:** Each plugin should include a README (usage, API, i18n, test, version, security notes) and follow a clear contribution guide.
- **中文：** 每個插件需附 README（用法、API、國際化、測試、版本、安全說明）並遵循明確的貢獻規範。
...
- **EN:** Monitor plugin execution time, memory usage, and errors. Integrate logging and metrics for observability.
- **中文：** 監控插件執行時間、記憶體用量與錯誤，建議整合日誌與指標，提升可觀測性。

```typescript
// Example: Plugin execution timing
...
> For enterprise and open-source collaboration, always prioritize plugin security, documentation, and observability. 持續強化插件安全、文件與可觀測性，讓國際協作更順暢。
