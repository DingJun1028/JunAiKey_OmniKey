...
export class RuneEngraftingAgent extends BaseAgent {
    ...
    protected async handleMessage(message: AgentMessage): Promise<AgentResponse> {
        console.log(`[RuneEngraftingAgent] Handling message: ${message.type} (Correlation ID: ${message.correlationId || 'N/A'})`);
        ...
        try {
            ...
            switch (message.type) {
                case 'register_rune':
                    ...
                    result = { message: `Rune ${message.payload.name} registered.` };
                ...
                default:
                    console.warn(`[RuneEngraftingAgent] Unknown message type: ${message.type}`);
                    return { success: false, error: `Unknown message type for RuneEngraftingAgent: ${message.type}` };
            }
        } catch (error: any) {
            console.error(`[RuneEngraftingAgent] Error handling message ${message.type}:`, error);
