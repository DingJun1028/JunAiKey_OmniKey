...
export class SacredRuneEngraver {
    ...
    private async loadAndRegisterUserRunes(userId: string): Promise<void> {
        console.log(`[SacredRuneEngraver] Loading and registering runes for user: ${userId}...`); // Updated log
        this.context.loggingService?.logInfo(`Loading and registering runes for user ${userId}`, { userId });
        ...
        try {
            ...
            console.log(`[SacredRuneEngraver] Found ${userRunes.length} user-owned runes.`); // Updated log
            ...
            for (const rune of userRunes) {
                try {
                    ...
                    // } else {
                    //      console.warn(`[SacredRuneEngraver] Client-side implementation not found for user rune ${rune.id} (${rune.name}). Cannot register.`); // Updated log
                    //      this.context.loggingService?.logWarning(`Client-side implementation not found for user rune`, { runeId: rune.id, userId });
                    // }

                    // For MVP, let's just log that we would register them if we had the implementation class here.
                    console.warn(`[SacredRuneEngraver] User rune ${rune.id} (${rune.name}) fetched from DB. Registration requires client-side implementation class.`); // Updated log
                    this.context.loggingService?.logWarning(`User rune fetched from DB, requires client-side implementation for registration`, { runeId: rune.id, userId });


                } catch (regError: any) {
                    console.error(`[SacredRuneEngraver] Error processing user rune ${rune.id} for registration:`, regError.message); // Updated log
                    this.context.loggingService?.logError(`Error processing user rune for registration`, { runeId: rune.id, userId, error: regError.message });
                }
            }
            console.log(`[SacredRuneEngraver] User rune loading and processing complete.`); // Updated log

        } catch (error: any) {
            console.error(`[SacredRuneEngraver] Error loading user runes for user ${userId}:`, error.message); // Updated log
            this.context.loggingService?.logError(`Error loading user runes`, { userId, error: error.message });
        }
    }
    // --- End New ---
    ...
    private clearUserRunesFromMemory(userId?: string | null): void {
        ...
        console.log(`[SacredRuneEngraver] Clearing runes for user: ${userId} from memory.`); // Updated log
        ...
        runeIdsToRemove.forEach(runeId => {
            ...
            console.log(`[SacredRuneEngraver] Cleared user rune ${runeId} from memory.`); // Updated log
        });
        console.log(`[SacredRuneEngraver] Cleared ${runeIdsToRemove.length} user runes from memory.`); // Updated log
    }
    // --- End New ---
    ...
    async registerRune(rune: Rune): Promise<void> {
        console.log(`[SacredRuneEngraver] Registering rune: ${rune.name} (${rune.id})`); // Updated log
        this.context.loggingService?.logInfo(`Registering rune: ${rune.name}`, { runeId: rune.id, userId: rune.user_id });
        ...
        if (this.runeImplementations.has(rune.id)) {
            console.warn(`[SacredRuneEngraver] Rune ${rune.id} is already registered. Overwriting.`); // Updated log
            this.context.loggingService?.logWarning(`Rune already registered: ${rune.id}`, { runeId: rune.id });
        }
        ...
        try {
            ...
            console.log(`[SacredRuneEngraver] Rune ${rune.id} registered successfully.`); // Updated log
            ...
            console.error(`[SacredRuneEngraver] Error registering rune ${rune.id}:`, error.message); // Updated log
            this.context.loggingService?.logError(`Error registering rune`, { runeId: rune.id, userId: rune.user_id, error: error.message });
    ...
    async listRunes(typeFilter?: Rune['type'], userId?: string): Promise<Rune[]> {
        ...
        try {
            ...
            console.log(`Fetched ${data.length} rune definitions for user ${userId || 'public'}.`);
            this.context.loggingService?.logInfo(`Fetched ${data.length} rune definitions successfully.`, { userId });
            ...
            // The `executeRuneAction` method will need to look up the implementation by ID.

            // To make the returned Rune objects usable for execution, we should attach the implementation reference.
            // This requires iterating through the fetched runes and finding their corresponding entry in `runeImplementations`.
            const usableRunes = runes.map(rune => {
                 ...
                 console.warn(`[SacredRuneEngraver] Rune ${rune.id} found in DB but not registered in memory. Cannot attach implementation.`); // Updated log
    ...
    async executeRuneAction(runeId: string, actionName: string, params?: any, userId?: string): Promise<any> {
        console.log(`[SacredRuneEngraver] Attempting to execute action \"${actionName}\" on rune \"${runeId}\" for user: ${userId}`); // Updated log
