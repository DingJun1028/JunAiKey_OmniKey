...
const SecurityAudit: React.FC = () => {
  ...
  const fetchAuditLogs = async () => {
   ...
   const fetchUserActions = async (query = '') => {
       ...
       try {
           ...
           // setError(`Failed to load user actions: ${err.message}`); // Don't set main error for just actions
  ...
  useEffect(() => {
    ...
    const handleAIAnalysis = async (entry: UserAction | SystemEvent) => {
        ...
        console.log(`Attempting AI analysis for entry: ${entry.id || entry.timestamp}`);
         // Simulate recording user action
        authorityForgingEngine?.recordAction({
        ...
        try {
            ...
            setAnalysisError(`Failed to perform AI analysis: ${err.message}`);
        } finally {
        ...
        <div className="mb-8 p-4 bg-neutral-700/50 rounded-lg"> {/* Added mb-8 */}
            ...
            {loadingLogs ? (
              ...
              <ul className="space-y-4">
                {auditLogs.map((log) => (
                  <li key={log.id || log.timestamp} className={`bg-neutral-600/50 p-4 rounded-md border-l-4 ${getSeverityColor(log.severity)}`}> {/* Use timestamp as fallback key */}
                    <div className="flex justify-between items-center mb-2">
                        <div className="flex items-center gap-3">
                            {getSeverityIcon(log.severity)}
                            <h4 className={`font-semibold ${getSeverityColor(log.severity)}`}>{log.type.replace(/_/g, ' ').toUpperCase()}</h4> {/* Display event type */}
                        </div>
            ...
            {loadingActions ? (
                ...
                <ul className="space-y-4">
                    {userActions.map((action) => (
                        <li key={action.id} className={`bg-neutral-600/50 p-4 rounded-md border-l-4 border-orange-500`}> {/* Use orange border for usage logs */}
                            <div className="flex justify-between items-center mb-2">
                                <div className="flex items-center gap-3">
                                    {getUserActionIcon(action.type)} {/* Use action type icon */}
                                    <h4 className={`font-semibold text-orange-200`}>Action Type: {action.type.replace(/_/g, ' ').toUpperCase()}</h4> {/* Display action type */}
                                </div>
                            ...
                            <small className="text-neutral-400 text-xs block mt-1">
                                ID: {action.id} | Timestamp: {new Date(action.timestamp).toLocaleString()}
                                 {action.user_id && ` | User: ${action.user_id}`}
                            </small>
