...
new Function ` which provides some isolation but is not
...
Executing;
script `, { userId, scriptLength: script.length, params });
...
`, { userId, details, scriptParams: params });
        ...
        // WARNING: Do NOT use `;
eval ` or `;
new Function ` with untrusted code in production
...
return (async () => {
    ...
    const { executeRuneAction, createTask, createAgenticFlow, saveKnowledge, searchKnowledge, sendNotification, updateGoalProgress, logInfo, logWarning, logError, currentUser } = context;
