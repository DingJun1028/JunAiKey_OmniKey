...
export class FileService {
    ...
    private async ensureBaseDirExists(): Promise<void> {
        try {
            ...
            console.log(`[FileService] Ensured base directory exists: ${this.baseDir}`);
        } catch (error: any) {
    ...
    private resolveSafePath(filePath: string): string {
        ...
        if (!absoluteFilePath.startsWith(absoluteBaseDir + path.sep) && absoluteFilePath !== absoluteBaseDir) {
            const errorMsg = `Attempted directory traversal: ${filePath}`;
            console.error(`[FileService] ${errorMsg}`);
    ...
    async readFile(filePath: string, userId: string): Promise<string> {
        console.log(`[FileService] Reading file: ${filePath} for user ${userId}`);
        this.context.loggingService?.logInfo(`Attempting to read file: ${filePath}`, { filePath, userId });
        ...
        try {
            ...
            console.log(`[FileService] Successfully read file: ${filePath}`);
            this.context.loggingService?.logInfo(`Successfully read file: ${filePath}`, { filePath, userId });
            return content;
        } catch (error: any) {
            console.error(`[FileService] Error reading file ${filePath}:`, error.message);
            this.context.loggingService?.logError(`Error reading file: ${filePath}`, { filePath, userId, error: error.message });
    ...
    async writeFile(filePath: string, content: string, userId: string): Promise<void> {
        console.log(`[FileService] Writing file: ${filePath} for user ${userId}`);
        this.context.loggingService?.logInfo(`Attempting to write file: ${filePath}`, { filePath, userId, contentSize: content.length });
        ...
        try {
            ...
            console.log(`[FileService] Successfully wrote file: ${filePath}`);
            this.context.loggingService?.logInfo(`Successfully wrote file: ${filePath}`, { filePath, userId });
            ...
            console.error(`[FileService] Error writing file ${filePath}:`, error.message);
            this.context.loggingService?.logError(`Error writing file: ${filePath}`, { filePath, userId, error: error.message });
    ...
    async updateFile(filePath: string, content: string, userId: string): Promise<void> {
        console.log(`[FileService] Updating file: ${filePath} for user ${userId}`);
    ...
    async mergeAndWriteFile(filePath: string, newContent: string, userId: string, mergeStrategy: 'append' | 'prepend' | 'replace' | 'diff' = 'append'): Promise<void> {
        console.log(`[FileService] Merging and writing file: ${filePath} for user ${userId} with strategy ${mergeStrategy}`);
        this.context.loggingService?.logInfo(`Attempting to merge and write file: ${filePath}`, { filePath, userId, mergeStrategy, newContentSize: newContent.length });
        ...
        try {
            ...
            try {
                ...
                if (readError.message.includes('ENOENT') || readError.message.includes('Failed to read file')) {
                    console.warn(`[FileService] File ${filePath} not found for merging. Creating new file.`);
                    existingContent = '';
                } else {
            ...
            switch (mergeStrategy) {
                ...
                case 'diff':
                    ...
                    break;
                default:
                    console.warn(`[FileService] Unknown merge strategy: ${mergeStrategy}. Appending content.`);
                    mergedContent = existingContent + newContent; // Fallback to append
                    break;
            }
            ...
            console.log(`[FileService] Successfully merged and wrote file: ${filePath}`);
            this.context.loggingService?.logInfo(`Successfully merged and wrote file: ${filePath}`, { filePath, userId, mergeStrategy });

        } catch (error: any) {
            console.error(`[FileService] Error merging and writing file ${filePath}:`, error.message);
            this.context.loggingService?.logError(`Error merging and writing file: ${filePath}`, { filePath, userId, mergeStrategy, error: error.message });
            throw new Error(`Failed to merge and write file ${filePath}: ${error.message}`);
        }
    }
    ...
    async deleteFile(filePath: string, userId: string): Promise<void> {
        console.log(`[FileService] Deleting file: ${filePath} for user ${userId}`);
        this.context.loggingService?.logInfo(`Attempting to delete file: ${filePath}`, { filePath, userId });
        ...
        try {
            ...
            this.context.loggingService?.logInfo(`Successfully deleted file: ${filePath}`, { filePath, userId });
            ...
            this.context.loggingService?.logError(`Error deleting file: ${filePath}`, { filePath, userId, error: error.message });
    ...
    async listFiles(dirPath: string = '/', userId: string): Promise<string[]> {
        console.log(`[FileService] Listing files in directory: ${dirPath} for user ${userId}`);
        this.context.loggingService?.logInfo(`Attempting to list files in directory: ${dirPath}`, { dirPath, userId });
        ...
        try {
            ...
            console.log(`[FileService] Successfully listed files in directory: ${dirPath}`);
            this.context.loggingService?.logInfo(`Successfully listed files in directory: ${dirPath}`, { dirPath, userId, count: entries.length });
            return entries;
        } catch (error: any) {
            console.error(`[FileService] Error listing files in directory ${dirPath}:`, error.message);
            this.context.loggingService?.logError(`Error listing files in directory: ${dirPath}`, { dirPath, userId, error: error.message });
    ...
    // TODO: Implement '\u91cd\u88fd' (Reset/Remake) - Could be a combination of delete and write.\n    // TODO: Implement '\u767c\u4f48/\u90e8\u7f72' (Publish/Deploy) - This is a complex process involving code, build, and deployment, likely orchestrated by SelfNavigationEngine or a dedicated DeploymentService, potentially using Runes (e.g., GitHub Actions Rune, Vercel Rune).\n    // TODO: Implement '\u63d0\u554f' (Ask a question) - This is a user interaction handled by JunAiAssistant.\n\n    // TODO: Integrate with SyncService for cloud synchronization of files.\n    // TODO: Implement permission checks based on user ID and file paths/metadata.\n    // TODO: This module is part of the Long-term Memory (\u6c38\u4e45\u8a18\u61b6) pillar.\n}\n```\n",
      "isBinary": false,
      "fullPath": "src/core/files/FileService.ts",
      "lastModified": 1750388229963
    },
