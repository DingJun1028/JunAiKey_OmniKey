...
let aiResponseOrResult = '';
let decidedAction = null;
let generatedStructure = null; // To hold structured output if needed
let actionExecutionResult = null; // To hold the result of the executed action
let actionExecutionError = null; // To hold the error if action execution fails
try {
    ...
    try {
        switch (decidedAction.action) {
            case 'create_task':
                ...
                actionExecutionResult = await this.context.selfNavigationEngine?.createTask(decidedAction.parameters.description, decidedAction.parameters.steps, // Use generated steps
                userId, // Pass userId
            ...
            case 'execute_rune':
                ...
                actionExecutionResult = await this.context.sacredRuneEngraver?.executeRuneAction(decidedAction.parameters.runeId, decidedAction.parameters.action, decidedAction.parameters.params, // Pass parameters to the rune action
                userId // Pass userId
                );
            ...
            case 'search_knowledge':
                ...
                actionExecutionResult = await this.context.knowledgeSync?.searchKnowledge(decidedAction.parameters.query, userId, // Pass userId
                ...
                if (actionExecutionResult && actionExecutionResult.length > 0) {
                    ...
                    actionExecutionResult.forEach((record) => {
            ...
            case 'sync_mobile_git':
                ...
                actionExecutionResult = await this.context.syncService?.syncMobileGitRepo(userId, // Pass userId
                direction, decidedAction.parameters?.details // Pass optional details like repoUrl
                );
            ...
            case 'create_agentic_flow':
                ...
                actionExecutionResult = await this.context.selfNavigationEngine?.createAgenticFlow(decidedAction.parameters, // Pass the full flow details (name, desc, entry_node_id, nodes, edges)
                userId // Pass userId
                );
                if (actionExecutionResult) {
            ...
            case 'update_goal_progress':
                ...
                actionExecutionResult = await this.context.goalManagementService?.updateKeyResultProgress(decidedAction.parameters.krId, decidedAction.parameters.currentValue, userId // Pass userId
                );
                if (actionExecutionResult) {
    ...
    if (this.context.knowledgeSync) {
        ...
        this.context.knowledgeSync.saveKnowledge(conversationRecord.question, conversationRecord.answer, userId, conversationRecord.source, conversationRecord.dev_log_details).catch(kbError => console.error('Failed to save conversation turn to KB:', kbError));
    }
    else {
