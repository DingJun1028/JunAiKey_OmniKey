...
\\\
    ...
    private async handleTaskCompleted(payload: any): Promise<void> {\\\
        ...
        if (payload?.taskId && payload?.userId && payload?.linked_kr_id) {\\\
            console.log(`[GoalManagementService] Task ${payload.taskId} completed, linked to KR ${payload.linked_kr_id}. Checking KR completion.`);\\\
            try {\\\
                ...
                console.error(`[GoalManagementService] Error checking KR completion after task ${payload.taskId} completed:`, error.message);\\\
                // Log the error\\\
            }\\\
        }\\\
    }\\\
...
\\\
            ...
            if (kr.current_value >= kr.target_value && kr.status !== 'completed') {\\\
                console.log(`KR ${kr.id} reached target value (${kr.current_value}/${kr.target_value}). Marking as completed.`);\\\
                this.context.loggingService?.logInfo(`KR reached target value. Marking as completed.`, { krId: kr.id, userId });\\\
\\\
...
\\\
                if (updateError) { throw updateError; }\\\
                if (!updatedKrData) {\\\
                    console.error(`Failed to retrieve updated KR data for ${kr.id} after marking completed.`);\\\
                    this.context.loggingService?.logError(`Failed to retrieve updated KR data after marking completed: ${kr.id}`, { userId });\\\
                    return undefined;\\\
                }\\\
\\\
                const updatedKr = updatedKrData as KeyResult;\\\
\\\
                console.log(`Key Result ${updatedKr.id} status updated to completed.`);\\\
                // Publish 'key_result_updated' event\\\
                this.context.eventBus?.publish('key_result_updated', updatedKr, userId);\\\
                this.context.loggingService?.logInfo(`KR status updated to completed: ${updatedKr.id}`, { userId: userId });\\\
\\\
...
\\\
                ...
                console.log(`KR ${kr.id} did not reach target value (${kr.current_value}/${kr.target_value}) or is already completed. No status update needed.`);\\\
                this.context.loggingService?.logInfo(`KR did not reach target or already completed.`, { krId: kr.id, userId });\\\
...
\\\
    ...
    // TODO: This module is part of the Bidirectional Sync Domain (\\u96d9\\u5410\\u540c\\u6b65\\u9818\\u57df) for persistence.\\\
\\\
    // --- New: Realtime Subscription Methods ---\\\
    /**\\\
     * Subscribes to real-time updates from the goals table for the current user.\\\
     * @param userId The user ID to filter updates by. Required.\\\
     */\\\
    subscribeToGoalsUpdates(userId: string): void {\\\
        console.log('[GoalManagementService] Subscribing to goals realtime updates for user:', userId);\\\
        this.context.loggingService?.logInfo('Subscribing to goals realtime updates', { userId });\\\
\\\
        if (this.goalsSubscription) {\\\
            console.warn('[GoalManagementService] Already subscribed to goals updates. Unsubscribing existing.');\\\
            this.unsubscribeFromGoalsUpdates();\\\
        }\\\
\\\
        this.goalsSubscription = this.supabase\\\
            .channel(`goals:user_id=eq.${userId}`) // Channel filtered by user_id\\\
            .on('postgres_changes', { event: '*', schema: 'public', table: 'goals', filter: `user_id=eq.${userId}` }, (payload) => {\\\
                console.log('[GoalManagementService] Realtime goal change received:', payload);\\\
                const goal = payload.new as Goal; // New data for INSERT/UPDATE\\\
                const oldGoal = payload.old as Goal; // Old data for UPDATE/DELETE\\\
                const eventType = payload.eventType as 'INSERT' | 'UPDATE' | 'DELETE';\\\
\\\n                // Publish an event via EventBus for other modules/UI to react\\\n                this.context.eventBus?.publish(`goal_${eventType.toLowerCase()}`, goal || oldGoal, userId); // e.g., 'goal_insert', 'goal_update', 'goal_delete'\\\n\\\n                // TODO: Notify SyncService about the remote change if SyncService is not listening to Realtime directly\\\n                // this.context.syncService?.handleRemoteDataChange('goalManagementService', eventType, goal || oldGoal, userId);\\\n            })\\\n            .subscribe((status, err) => {\\\n                 console.log('[GoalManagementService] Goal subscription status:', status);\\\n                 if (status === 'CHANNEL_ERROR') {\\\n                     console.error('[GoalManagementService] Goal subscription error:', err);\\\n                     this.context.loggingService?.logError('Goal subscription error', { userId, error: err?.message });\\\n                 }\\\n            });\\\n    }\\\n\\\n    /**\\\n     * Unsubscribes from goals real-time updates.\\\n     */\\\
    unsubscribeFromGoalsUpdates(): void {\\\
        if (this.goalsSubscription) {\\\
            console.log('[GoalManagementService] Unsubscribing from goals realtime updates.');\\\
            this.context.loggingService?.logInfo('Unsubscribing from goals realtime updates');\\\
            this.supabase.removeChannel(this.goalsSubscription);\\\
            this.goalsSubscription = null;\\\
        }\\\
    }\\\n\\\n    /**\\\
     * Subscribes to real-time updates from the key_results table for the current user's goals.\\\
     * @param userId The user ID to filter updates by. Required.\\\
     */\\\
    subscribeToKeyResultsUpdates(userId: string): void {\\\
        console.log('[GoalManagementService] Subscribing to key result realtime updates for user:', userId);\\\
        this.context.loggingService?.logInfo('Subscribing to key result realtime updates', { userId });\\\
\\\n        if (this.keyResultsSubscription) {\\\
            console.warn('[GoalManagementService] Already subscribed to key result updates. Unsubscribing existing.');\\\
            this.unsubscribeFromKeyResultsUpdates();\\\
        }\\\
\\\n        // Note: RLS on key_results should ensure user can only see KRs for their goals.\\\
        // Rely on RLS and subscribe to all changes on the table.\\\
        this.keyResultsSubscription = this.supabase\\\
            .channel('key_results') // Subscribe to all changes on the table\\\
            .on('postgres_changes', { event: '*', schema: 'public', table: 'key_results' }, async (payload) => { // No filter here, rely on RLS\\\
                console.log('[GoalManagementService] Realtime key result change received:', payload);\\\
                const kr = payload.new as KeyResult & { goal_id: string }; // New data for INSERT/UPDATE\\\
                const oldKr = payload.old as KeyResult & { goal_id: string }; // Old data for UPDATE/DELETE\\\
                const eventType = payload.eventType as 'INSERT' | 'UPDATE' | 'DELETE';\\\
\\\n                // To publish with user_id, we need the user_id of the parent goal.\\\
                const krWithGoalId = kr || oldKr;\\\
                if (krWithGoalId?.goal_id) {\\\
                    const parentGoal = await this.getGoalById(krWithGoalId.goal_id, userId); // Fetch parent goal to get user_id\\\
                    if (parentGoal?.user_id === userId) { // Ensure it belongs to the current user\\\
                         // Publish an event via EventBus for other modules/UI to react\\\
                         this.context.eventBus?.publish(`key_result_${eventType.toLowerCase()}`, kr || oldKr, userId); // e.g., 'key_result_insert', 'key_result_update', 'key_result_delete'\\\
\\\n                         // TODO: Notify SyncService about the remote change\\\
                         // this.context.syncService?.handleRemoteDataChange('goalManagementService', eventType, kr || oldKr, userId);\\\
                    } else {\\\
                         console.log('[GoalManagementService] Received key result change for a goal not belonging to the current user (filtered by RLS or client-side).');\\\
                    }\\\
                } else {\\\
                     console.warn('[GoalManagementService] Received key result change without goal_id.');\\\
                }\\\
            })\\\
            .subscribe((status, err) => {\\\
                 console.log('[GoalManagementService] Key result subscription status:', status);\\\
                 if (status === 'CHANNEL_ERROR') {\\\
                     console.error('[GoalManagementService] Key result subscription error:', err);\\\
                     this.context.loggingService?.logError('Key result subscription error', { userId, error: err?.message });\\\
                 }\\\
            });\\\
    }\\\
\\\n    /**\\\
     * Unsubscribes from key result real-time updates.\\\
     */\\\
    unsubscribeFromKeyResultsUpdates(): void {\\\
        if (this.keyResultsSubscription) {\\\
            console.log('[GoalManagementService] Unsubscribing from key result realtime updates.');\\\
            this.context.loggingService?.logInfo('Unsubscribing from key result realtime updates');\\\
            this.supabase.removeChannel(this.keyResultsSubscription);\\\
            this.keyResultsSubscription = null;\\\
        }\\\
    }\\\
    // --- End New ---\\\
}\\\
