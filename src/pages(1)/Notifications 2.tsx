            ...
            {loading ? (
              ...
              <ul className="space-y-4">
                {notifications.map((notification) => (
                  <li key={notification.id} className={`bg-neutral-600/50 p-4 rounded-md border-l-4 ${getNotificationTypeColor(notification.type)}`}>
                    <div className="flex justify-between items-center mb-2">
                        <div className="flex items-center gap-3">
                            {getNotificationTypeIcon(notification.type)}
                            <h4 className={`font-semibold ${getNotificationTypeColor(notification.type)}`}>{notification.type.toUpperCase()}</h4> {/* Display event type */}
                        </div>
                         {/* Mark as Read Button */}
                         <button
                         ...
                         >
                            <MailOpen size={16} className={`inline-block mr-1 ${markingAsReadId === notification.id ? 'animate-spin' : ''}`}/>
