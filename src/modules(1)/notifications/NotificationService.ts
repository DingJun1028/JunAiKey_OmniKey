...
export class NotificationService {
    ...
    private async handleNewKnowledgeEvent(payload: any): Promise<void> {
        ...
        if (payload?.user_id) {
            await this.sendNotification({
                ...
                type: 'info',
                message: `New knowledge record added: \"${payload.question.substring(0, 50)}...\"`,
                channel: 'ui', // Send to UI by default
                details: { recordId: payload.id, question: payload.question },
            });
        }
    }

     /**
     * Handles a task completed event to send a notification.
     * @param payload The event payload (Task).
     */
    private async handleTaskCompletedEvent(payload: any): Promise<void> {
     ...
     /**
     * Handles a task failed event to send a notification.
     * @param payload The event payload (Task).
     */
    private async handleTaskFailedEvent(payload: any): Promise<void> {
     ...
     /**
     * Handles an insight generated event to send a notification.
     * @param payload The event payload (EvolutionaryInsight).
     */
    private async handleInsightGeneratedEvent(payload: any): Promise<void> {
     ...
     /**
     * Handles an Agentic Flow completed event to send a notification.
     * @param payload The event payload (AgenticFlow).
     */
    private async handleAgenticFlowCompletedEvent(payload: any): Promise<void> {
     ...
     /**
     * Handles an Agentic Flow failed event to send a notification.
     * @param payload The event payload (AgenticFlow).
     */
    private async handleAgenticFlowFailedEvent(payload: any): Promise<void> {
     ...
     */
    async sendNotification(notificationDetails: Omit<Notification, 'id' | 'timestamp' | 'is_read'>): Promise<Notification | null> {
        ...
        const newNotificationData: Omit<Notification, 'id' | 'timestamp' | 'is_read'> = {
            ...notificationDetails,
     ...
     */
    async getNotifications(userId: string, isRead?: boolean, channel?: Notification['channel'], limit: number = 100): Promise<Notification[]> {
    ...
    async markAsRead(notificationId: string, userId: string): Promise<Notification | undefined> {
        ...
        try {
            const { data, error } = await this.supabase
                .from('notifications')
                .update({ is_read: true } as Partial<Notification>)
     ...
     */
    subscribeToUiNotifications(callback: UiNotificationCallback): () => void {
        console.log('[NotificationService] UI subscribing to notifications.');
        this.uiNotificationListeners.push(callback);
        // Immediately notify the new listener with the current cache
        callback([...this.unreadUiNotificationsCache]); // Pass a copy
    ...
    /**
     * Notifies all registered UI listeners with the current list of unread UI notifications.
     */
     ...
     */
    subscribeToNotificationUpdates(userId: string): void {
        ...
        this.notificationSubscription = this.supabase
            ...
            .on('postgres_changes', { event: '*', schema: 'public', table: 'notifications', filter: `user_id=eq.${userId}` }, (payload) => {
                ...
                const notification = (payload.new || payload.old) as Notification; // New data for INSERT/UPDATE, old for DELETE
     ...
     */
    unsubscribeFromNotificationUpdates(): void {
        if (this.notificationSubscription) {
    ...
    // TODO: Implement methods for managing notification settings (e.g., enable/disable channels for types).
    // TODO: Implement methods for sending notifications via Email, Push, Webhook channels.
