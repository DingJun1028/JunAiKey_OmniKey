...
export async function syncBoostspaceTasks() {
  eventBus?.emit?.('sync:start', { source: 'boostspace', ts: Date.now() });
  // 1. 拉取 Boost.space 任務
  ...
  const syncResult = { synced: toUpsert.length, total: remoteTasks.length, lastSyncAt: new Date().toISOString(), tasks: remoteTasks };
  eventBus?.emit?.('sync:done', { source: 'boostspace', ...syncResult });
  console.log(`[BoostspaceSync] 同步完成，新增/更新 ${toUpsert.length} 筆，總數 ${remoteTasks.length}`);
  // 4. 更新 lastSyncAt 狀態
  await supabase.from('sync_status').upsert({ source: 'boostspace', last_sync_at: syncResult.lastSyncAt }, { onConflict: 'source' });
  return syncResult;
}
