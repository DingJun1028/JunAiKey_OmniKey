...
for (user; $; { userId })
    : $;
{
    eventPayload?.type || 'Unknown Type';
}
`);\
...
potential;
trigger;
event `, { userId, eventType: eventPayload?.type, eventSource: eventPayload?.source, correlationId: eventPayload?.context?.correlationId });\
\
...
Triggering;
execution. `, { userId, eventType: eventPayload?.type, matchingAbilityIds: matchingAbilities.map(a => a.id) });\
\
...
handling;
trigger;
event `, { userId, eventType: eventPayload?.type, error: error.message });\
