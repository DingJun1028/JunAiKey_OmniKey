...
""(__makeTemplateObject(["typescript\n// src/pages/Calendar.tsx\n// Calendar Page\n// Displays and manages user's calendar events.\n// --- New: Create a page for the Calendar UI ---// --- New: Implement fetching and displaying calendar events ---// --- New: Add UI for creating, editing, and deleting events ---// --- New: Add Realtime Updates for calendar_events ---import React, { useEffect, useState } from 'react';import { CalendarService } from '../core/calendar/CalendarService'; // To fetch and manage eventsimport { AuthorityForgingEngine } from '../core/authority/AuthorityForgingEngine'; // Use for action recordingimport { CalendarEvent } from '../interfaces'; // Import CalendarEvent typeimport { CalendarDays, ChevronDown, ChevronUp, Trash2, Edit, PlusCircle, Save, Loader2, Info, Star, StarHalf, StarOff, Tag, MapPin, Link as LinkIcon } from 'lucide-react'; // Import icons including MapPin, LinkIcon// Access core modules from the global window object (for MVP simplicity)// In a real app, use React Context or dependency injectiondeclare const window: any;const calendarService: CalendarService = window.systemContext?.calendarService; // The Calendar Serviceconst authorityForgingEngine: any = window.systemContext?.authorityForgingEngine; // Use any for action recording (\u6B0A\u80FD\u935B\u9020)const systemContext: any = window.systemContext; // Access the full context for currentUserconst Calendar: React.FC = () => {  const [events, setEvents] = useState<CalendarEvent[]>([]); // State to hold calendar events  const [loading, setLoading] = useState(true);  const [error, setError] = useState<string | null>(null);  const [expandedEvents, setExpandedEvents] = useState<Record<string, boolean>>({}); // State to track expanded events  // --- State for creating new event ---  const [isCreatingEvent, setIsCreatingEvent] = useState(false);  const [newEventTitle, setNewEventTitle] = useState('');  const [newEventDescription, setNewEventDescription] = useState('');  const [newEventStartTime, setNewEventStartTime] = useState(''); // ISO 8601 string  const [newEventEndTime, setNewEventEndTime] = useState(''); // ISO 8601 string  const [newEventAllDay, setNewEventAllDay] = useState(false);  const [newEventLocation, setNewEventLocation] = useState('');  const [newEventUrl, setNewEventUrl] = useState('');  const [newEventTags, setNewEventTags] = useState<string[]>([]);  const [isSavingEvent, setIsSavingEvent] = useState(false);  // --- End New ---  // --- State for editing event ---  const [editingEvent, setEditingEvent] = useState<CalendarEvent | null>(null); // The event being edited  const [editingEventTitle, setEditingEventTitle] = useState('');  const [editingEventDescription, setEditingEventDescription] = useState('');  const [editingEventStartTime, setEditingEventStartTime] = useState(''); // ISO 8601 string  const [editingEventEndTime, setEditingEventEndTime] = useState(''); // ISO 8601 string  const [editingEventAllDay, setEditingEventAllDay] = useState(false);  const [editingEventLocation, setEditingEventLocation] = useState('');  const [editingEventUrl, setEditingEventUrl] = useState('');  const [editingEventTags, setEditingEventTags] = useState<string[]>([]);  const [isUpdatingEvent, setIsUpdatingEvent] = useState(false);  // --- End New ---  const fetchEvents = async () => {       const userId = systemContext?.currentUser?.id;       if (!calendarService || !userId) {            setError(\"CalendarService module not initialized or user not logged in.\");            setLoading(false);            return;        }      setLoading(true);      setError(null); // Clear main error when fetching      try {          // Fetch calendar events for the current user          // For MVP, fetch all events. In a real calendar, you'd fetch for a specific date range.          const userEvents = await calendarService.getEvents(userId); // Pass user ID          setEvents(userEvents);      } catch (err: any) {          console.error('Error fetching calendar events:', err);          setError("], ["typescript\n// src/pages/Calendar.tsx\n// Calendar Page\n// Displays and manages user's calendar events.\n// --- New: Create a page for the Calendar UI ---\\\n// --- New: Implement fetching and displaying calendar events ---\\\n// --- New: Add UI for creating, editing, and deleting events ---\\\n// --- New: Add Realtime Updates for calendar_events ---\\\n\\\n\\\nimport React, { useEffect, useState } from 'react';\\\nimport { CalendarService } from '../core/calendar/CalendarService'; // To fetch and manage events\\\nimport { AuthorityForgingEngine } from '../core/authority/AuthorityForgingEngine'; // Use for action recording\\\nimport { CalendarEvent } from '../interfaces'; // Import CalendarEvent type\\\nimport { CalendarDays, ChevronDown, ChevronUp, Trash2, Edit, PlusCircle, Save, Loader2, Info, Star, StarHalf, StarOff, Tag, MapPin, Link as LinkIcon } from 'lucide-react'; // Import icons including MapPin, LinkIcon\\\n\\\n\\\n// Access core modules from the global window object (for MVP simplicity)\\\n// In a real app, use React Context or dependency injection\\\ndeclare const window: any;\\\nconst calendarService: CalendarService = window.systemContext?.calendarService; // The Calendar Service\\\nconst authorityForgingEngine: any = window.systemContext?.authorityForgingEngine; // Use any for action recording (\\u6b0a\\u80fd\\u935b\\u9020)\\\nconst systemContext: any = window.systemContext; // Access the full context for currentUser\\\n\\\n\\\nconst Calendar: React.FC = () => {\\\n  const [events, setEvents] = useState<CalendarEvent[]>([]); // State to hold calendar events\\\n  const [loading, setLoading] = useState(true);\\\n  const [error, setError] = useState<string | null>(null);\\\n  const [expandedEvents, setExpandedEvents] = useState<Record<string, boolean>>({}); // State to track expanded events\\\n\\\n  // --- State for creating new event ---\\\n  const [isCreatingEvent, setIsCreatingEvent] = useState(false);\\\n  const [newEventTitle, setNewEventTitle] = useState('');\\\n  const [newEventDescription, setNewEventDescription] = useState('');\\\n  const [newEventStartTime, setNewEventStartTime] = useState(''); // ISO 8601 string\\\n  const [newEventEndTime, setNewEventEndTime] = useState(''); // ISO 8601 string\\\n  const [newEventAllDay, setNewEventAllDay] = useState(false);\\\n  const [newEventLocation, setNewEventLocation] = useState('');\\\n  const [newEventUrl, setNewEventUrl] = useState('');\\\n  const [newEventTags, setNewEventTags] = useState<string[]>([]);\\\n  const [isSavingEvent, setIsSavingEvent] = useState(false);\\\n  // --- End New ---\\\n\\\n  // --- State for editing event ---\\\n  const [editingEvent, setEditingEvent] = useState<CalendarEvent | null>(null); // The event being edited\\\n  const [editingEventTitle, setEditingEventTitle] = useState('');\\\n  const [editingEventDescription, setEditingEventDescription] = useState('');\\\n  const [editingEventStartTime, setEditingEventStartTime] = useState(''); // ISO 8601 string\\\n  const [editingEventEndTime, setEditingEventEndTime] = useState(''); // ISO 8601 string\\\n  const [editingEventAllDay, setEditingEventAllDay] = useState(false);\\\n  const [editingEventLocation, setEditingEventLocation] = useState('');\\\n  const [editingEventUrl, setEditingEventUrl] = useState('');\\\n  const [editingEventTags, setEditingEventTags] = useState<string[]>([]);\\\n  const [isUpdatingEvent, setIsUpdatingEvent] = useState(false);\\\n  // --- End New ---\\\n\\\n\\\n  const fetchEvents = async () => {\\\n       const userId = systemContext?.currentUser?.id;\\\n       if (!calendarService || !userId) {\\\n            setError(\\\"CalendarService module not initialized or user not logged in.\\\");\\\n            setLoading(false);\\\n            return;\\\n        }\\\n      setLoading(true);\\\n      setError(null); // Clear main error when fetching\\\n      try {\\\n          // Fetch calendar events for the current user\\\n          // For MVP, fetch all events. In a real calendar, you'd fetch for a specific date range.\\\n          const userEvents = await calendarService.getEvents(userId); // Pass user ID\\\n          setEvents(userEvents);\\\n      } catch (err: any) {\\\n          console.error('Error fetching calendar events:', err);\\\n          setError("]));
Failed;
to;
load;
calendar;
...
");      } finally {          setLoading(false);      }  };  useEffect(() => {    // Fetch data when the component mounts or when the user changes    if (systemContext?.currentUser?.id) {        fetchEvents(); // Fetch all events on initial load    }    // --- New: Subscribe to realtime updates for calendar_events ---    let unsubscribeInsert: (() => void) | undefined;    let unsubscribeUpdate: (() => void) | undefined;    let unsubscribeDelete: (() => void) | undefined;    if (calendarService?.context?.eventBus) { // Check if CalendarService and its EventBus are available        const eventBus = calendarService.context.eventBus;        const userId = systemContext?.currentUser?.id;        // Subscribe to insert events        unsubscribeInsert = eventBus.subscribe('calendar_event_insert', (payload: CalendarEvent) => {            if (payload.user_id === userId) {                console.log('Calendar page received calendar_event_insert event:', payload);                // Add the new event and keep sorted by start_timestamp (newest first)                setEvents(prevEvents => [payload, ...prevEvents].sort((a, b) => new Date(b.start_timestamp).getTime() - new Date(a.start_timestamp).getTime()));            }        });        // Subscribe to update events        unsubscribeUpdate = eventBus.subscribe('calendar_event_update', (payload: CalendarEvent) => {             if (payload.user_id === userId) {                 console.log('Calendar page received calendar_event_update event:', payload);                 // Update the specific event in the state                 setEvents(prevEvents => prevEvents.map(event => event.id === payload.id ? payload : event));             }         });        // Subscribe to delete events        unsubscribeDelete = eventBus.subscribe('calendar_event_delete', (payload: { eventId: string, userId: string }) => {             if (payload.userId === userId) {                 console.log('Calendar page received calendar_event_delete event:', payload);                 // Remove the deleted event from the state                 setEvents(prevEvents => prevEvents.filter(event => event.id !== payload.eventId));             }         });    }    // --- End New ---    return () => {        // Unsubscribe on component unmount        unsubscribeInsert?.();        unsubscribeUpdate?.();        unsubscribeDelete?.();    };  }, [systemContext?.currentUser?.id, calendarService]); // Re-run effect when user ID or service changes    const toggleExpandEvent = (eventId: string) => {        setExpandedEvents(prevState => ({            ...prevState,            [eventId]: !prevState[eventId]        }));    };    // --- New: Handle Create Event ---    const handleCreateEvent = async (e: React.FormEvent) => {        e.preventDefault();        const userId = systemContext?.currentUser?.id;        if (!calendarService || !userId || !newEventTitle.trim() || !newEventStartTime) {            alert(\"CalendarService module not initialized, user not logged in, or title/start time is empty.\");            return;        }        setIsSavingEvent(true);        setError(null);        try {            const eventDetails: Omit<CalendarEvent, 'id' | 'created_at' | 'updated_at'> = {                user_id: userId, // Ensure user_id is included                title: newEventTitle.trim(),                description: newEventDescription.trim() || undefined,                start_timestamp: newEventStartTime,                end_timestamp: newEventEndTime || undefined,                all_day: newEventAllDay,                location: newEventLocation.trim() || undefined,                url: newEventUrl.trim() || undefined,                source: 'manual', // Manually created event                tags: newEventTags.map(tag => tag.trim()).filter(tag => tag),            };            // Create the event            const createdEvent = await calendarService.createEvent(eventDetails, userId); // Pass eventDetails and userId            if (createdEvent) {                alert(";
Calendar;
event;
