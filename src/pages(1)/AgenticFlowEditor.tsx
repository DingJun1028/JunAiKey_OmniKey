...
const AgenticFlowEditor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // --- New: Handle Add Node ---\
    const handleAddNode = () => {
        const newNodeId = `node-${nodes.length + 1}`; // Simple sequential ID
        const newNode: Node = {
            id: newNodeId,
            ...
            data: {
                ...
                nodeData: { // Store placeholder node data
                    ...
                    description: `New ${newNodeId}`, // Default description
    ...
    const handleSaveFlow = async () => {
        ...
        if (!entryNodeExists) {
             ...
             alert(`Cannot save flow: the original entry node '${flow.entry_node_id}' was deleted. Please set a new entry node before saving.`);
        ...
        // The `updateAgenticFlowStructure` method handles updating nodes/edges based on the provided lists.
          ...
          <p className="text-neutral-400">Loading flow details...</p>
            ...
            <>
                <div className="flex items-center gap-4 mb-4">
                    <Link to={`/flows/${flow.id}`} className="text-neutral-400 hover:text-white transition">
                        <ArrowLeft size={24} />
                    </Link>
                ...
                <div className="flex-grow border border-neutral-600 rounded-md overflow-hidden" style={{ height: '500px' }}> {/* Fixed height for visualization */}\
                    <ReactFlowProvider> {/* Wrap with provider */}\
                        <ReactFlow\
                            ...
                            onNodeClick={(event, node) => handleEditNodeClick(node.data.nodeData)} // Handle click to edit node\
                            onEdgeClick={(event, edge) => handleEditEdgeClick(event, edge)} // Handle click to edit edge\
                        >\
                            <MiniMap />\
                            <Controls />\
                ...
                {(editingNode || editingEdge) && (\
                    <div className="mt-4 p-4 bg-neutral-700/50 rounded-lg">\
                        <div className="flex justify-between items-center mb-3">\
                            <h3 className="text-xl font-semibold text-blue-300">{editingNode ? `Edit Node: ${editingNode.id}` : `Edit Edge: ${editingEdge?.id}`}</h3>\
                            ...
                            <div className="space-y-4">\
                                ...
                                <div>\
                                    ...
                                    <select\
                                        ...
                                        onChange={(e) => setNodeProperties({ ...nodeProperties, type: e.target.value as AgenticFlowNode['type'], action: undefined, decision_logic: undefined })} // Reset action/logic when type changes\
                                        disabled={isSaving}\
                                        required\
                                    >\
                                ...
                                {(nodeProperties.type === 'task_step' || nodeProperties.type === 'rune_action' || nodeProperties.type === 'ability_execution') && ((\
                                     <div className="p-3 bg-neutral-600/50 rounded-md">\
                                         ...
                                         <ActionEditor\
                                             ...
                                             onChange={(newAction) => setNodeProperties({ ...nodeProperties, action: newAction, decision_logic: undefined })} // Update action, clear decision logic\
                                             disabled={isSaving}\
                                    ...
                                    <div className="p-3 bg-neutral-600/50 rounded-md space-y-2">\
                                        ...
                                        <div>\
                                            ...
                                            <input\
                                                ...
                                                value={nodeProperties.action?.details?.subFlowId || ''} // Assuming subFlowId is in action.details\
                                                onChange={(e) => setNodeProperties({ ...nodeProperties, action: { type: 'runSubFlow', details: { ...nodeProperties.action?.details, subFlowId: e.target.value } } })} // Update action details\
                                         ...
                                         <div className="mt-2 p-2 bg-neutral-700/50 rounded-md">\
                                             <label htmlFor="subflow-params-json" className="block text-neutral-400 text-xs font-semibold mb-1">Parameters (JSON):</label>\
                                              <textarea\
                                                 ...
                                                 className={`w-full p-1 rounded-md bg-neutral-800 text-white border ${propertiesError ? 'border-red-500' : 'border-neutral-600'} focus:outline-none focus:ring-1 focus:ring-blue-500 font-mono text-xs`}\
                                                 value={JSON.stringify(nodeProperties.action?.details?.params || {}, null, 2)}\
                                                 onChange={(e) => {\
                                                     try {\
                                                         const params = JSON.parse(e.target.value);\
                                                         setNodeProperties({ ...nodeProperties, action: { type: 'runSubFlow', details: { ...nodeProperties.action?.details, params } } }); // Update action details\
                                                         setPropertiesError(null); // Clear error if parsing is successful\
                                                     } catch (parseError: any) {\
                                                         setPropertiesError(`Invalid JSON: ${parseError.message}`);\
                                                     }\
                                    ...
                                    <div className="p-3 bg-neutral-600/50 rounded-md space-y-2">\
                                        ...
                                        <div>\
                                            ...
                                            <input\
                                                ...
                                                type="text"\
                                                className="w-full p-1 rounded-md bg-neutral-800 text-white border border-neutral-600 focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm"\
                                                value={nodeProperties.action?.details?.prompt || ''} // Assuming prompt is in action.details\
                                                onChange={(e) => setNodeProperties({ ...nodeProperties, action: { type: 'waitForUserInput', details: { ...nodeProperties.action?.details, prompt: e.target.value } } })} // Update action details\
                            ...
                            <div className="space-y-4">\
                                ...
                                <div>\
                                    <label htmlFor="edge-condition" className="block text-neutral-300 text-sm font-semibold mb-1">Condition (JSON):</label>\
                                     <textarea\
                                        id="edge-condition"\
                                        className={`w-full p-2 rounded-md bg-neutral-800 text-white border ${propertiesError ? 'border-red-500' : 'border-neutral-600'} focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-xs`}\
...
\
                        <div className="flex gap-4 justify-end mt-4">\
                            ...
                            <button\
                                ...
                                className="px-6 py-2 bg-green-600 text-white font-semibold rounded-md hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed"\
                                disabled={isSaving || !!propertiesError} // Disable if saving or there's a JSON error\
                            >\
                                Save Properties\
                            </button>\
                        </div>\
