...
```mermaid
flowchart LR
  A[authority forge engine] --> B[forge authority]
  A --> C[get all authorities]
  A --> D[forge module]
```
...
- on each sync, compare `updated_at` or `version` fields between source and target; only trigger write if state changes.
- mark source on write (e.g., `source: boostspace` or `source: supabase`) to avoid duplicate triggers.
