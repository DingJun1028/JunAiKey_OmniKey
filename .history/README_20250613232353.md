...
>
> - **Open/Closed Principle (OCP)**: All extension points (formatters, agents, middleware) follow OCP—open for extension, closed for modification. This ensures core logic is stable while allowing new features via plugins. 所有擴展點（格式工廠、代理、中間件）皆遵循開放封閉原則，核心穩定、擴展靈活。
> - **Plugin Registration & Auto-Discovery**: For large-scale collaboration and multi-platform scenarios, use auto-discovery (e.g., dynamic import, registry pattern) to load plugins. 插件註冊與自動發現（如動態 import、註冊表模式）適用於大型協作與多平台擴展。
> - **TypeScript Type Safety**: Strict interface/type definitions ensure extensibility safety and cross-team clarity. 型別嚴格有助於國際團隊協作與維護。
> - **Bilingual Documentation**: For international teams, always provide bilingual comments and docs. 國際協作建議雙語註解與說明。
...
> **Best Practice | 最佳實踐**
>
> - Centralize plugin registration for maintainability. 插件註冊集中管理。
> - Document all extension points and provide usage examples. 所有擴展點需有文件與範例。
> - Encourage code review and automated tests for all plugins. 插件需經過審查與自動化測試。
> - Use English and 中文 for all key comments and docs. 重要註解與說明建議雙語。
