...
export class TemplateService {
    ...
    async getTemplates(userId: string, typeFilter?: Template['type'], tagsFilter?: string[]): Promise<Template[]> {
        console.log(`[TemplateService] Retrieving templates for user: ${userId}, type: ${typeFilter || 'all'}, tags: ${tagsFilter || 'any'}`);
        ...
        try {
            ...
            let dbQuery = this.supabase
                ...
                .or(`user_id.eq.${userId},is_public.eq.true`); // Fetch user's own templates OR public templates
            ...
            console.log(`Fetched ${templates.length} templates for user ${userId}.`);
            this.context.loggingService?.logInfo(`Fetched ${templates.length} templates successfully.`, { userId });
    ...
    async getTemplateById(templateId: string, userId: string): Promise<Template | undefined> {
        ...
        this.context.loggingService?.logInfo(`Attempting to fetch template ${templateId}`, { id: templateId, userId });
        ...
        try {
            // Fetch template from Supabase by ID and check if it's public OR owned by the user
              ...
              console.log(`Fetched template ${templateId} for user ${userId}.`);
              this.context.loggingService?.logInfo(`Fetched template ${templateId} successfully.`, { id: templateId, userId: userId });

              return template;

        } catch (error: any) {
            console.error(`Error fetching template ${templateId} from Supabase:`, error);
            this.context.loggingService?.logError(`Failed to fetch template ${templateId}`, { id: templateId, userId: userId, error: error.message });
            return undefined;
        }
    }
    ...
    async updateTemplate(templateId: string, updates: Partial<Omit<Template, 'id' | 'user_id' | 'created_at' | 'updated_at'>>, userId: string): Promise<Template | undefined> {
        console.log(`Updating template ${templateId} in Supabase for user ${userId}...`, updates);
        this.context.loggingService?.logInfo(`Attempting to update template ${templateId}`, { id: templateId, updates, userId });
        ...
        try {
            ...
            // Filter by ID and user_id to ensure ownership and that it's not a public template (unless admin)
              ...
              if (!data) { // Should not happen if RLS is correct and ID/user_id match
                   console.warn(`Template ${templateId} not found or does not belong to user ${userId} (or is public) for update.`);
                   this.context.loggingService?.logWarning(`Template not found or user mismatch for update: ${templateId}`, { userId });
                   return undefined;
              }

              const updatedTemplate = data as Template;

            console.log(`Template ${templateId} updated in Supabase.`);
            ...
            this.context.loggingService?.logInfo(`Template updated successfully: ${templateId}`, { id: templateId, userId: userId });
            ...
            this.context.loggingService?.logError(`Failed to update template ${templateId}`, { id: templateId, updates, userId: userId, error: error.message });
            throw error; // Re-throw the error
        }
    }
    ...
    async deleteTemplate(templateId: string, userId: string): Promise<boolean> {
        console.log(`Deleting template ${templateId} from Supabase for user ${userId}...`);
        this.context.loggingService?.logInfo(`Attempting to delete template ${templateId}`, { id: templateId, userId });
...
\
              if (deleted) {\
                  console.log(`Template ${templateId} deleted from Supabase.`);\
                  ...
                  this.context.loggingService?.logInfo(`Template deleted successfully: ${templateId}`, { id: templateId, userId: userId });\
              } else {\
                  console.warn(`Template ${templateId} not found for deletion or user mismatch (or is public).`);\
                  this.context.loggingService?.logWarning(`Template not found for deletion or user mismatch: ${templateId}`, { id: templateId, userId });\
              }\
              return deleted;\
\
        } catch (error: any) {\
            console.error(`Failed to delete template ${templateId}:`, error);\
            this.context.loggingService?.logError(`Failed to delete template ${templateId}`, { id: templateId, userId: userId, error: error.message });\
...
\
                if (isRelevant) {\
                    console.log(`[TemplateService] Processing relevant template change (${eventType}): ${relevantTemplate.id}`);\
\
                    // Publish an event via EventBus for other modules/UI to react\
                    this.context.eventBus?.publish(`template_${eventType.toLowerCase()}`, template || payload.old, userId); // e.g., 'template_insert', 'template_update', 'template_delete'\
\
